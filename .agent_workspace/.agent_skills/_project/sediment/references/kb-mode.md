...
> 库的读写工具（kb_*）挂在 `knowledge-base` 子 agent 上——本模式不直接接触这些工具，落库一律通过调用 `knowledge-base` 委派工具完成。
...
3. **委派落库**：调用一次 `knowledge-base` 委派工具处理全部候选。description 里写清：
...
| 项 | 说明 |
|---|---|
| 委派方式 | 调用 `knowledge-base` 委派工具，一次委派带上全部候选，description 写全素材与要求 |
