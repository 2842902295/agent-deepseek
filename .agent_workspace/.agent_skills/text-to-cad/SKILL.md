...
| Polyline → Region | `polyline_to_region(doc, ms, pl)`（见下方辅助函数） | **勿裸调 `ms.AddRegion`**——中望 COM 封送必失败（坑位 1）；辅助函数自动回退 `_REGION` 命令 |

#### COM 3D 辅助函数（实战验证，直接复制使用）

以下函数全部来自真实项目实测（对应下方「ZWCAD COM 实战坑位清单」），**写 COM 3D 代码时一律优先复用，不要裸调原始 API**：

```python
import time

def polyline_to_region(doc, ms, pl):
    """封闭多段线 → Region（AutoCAD / 中望通用）。
    坑位 1：中望的 ms.AddRegion 不接受 comtypes 对象数组，裸调必失败。
    策略：先试 AddRegion（AutoCAD 通常可用），失败回退 _REGION 命令行转换。
    要求：pl 必须是刚加入模型空间的最后一个实体。"""
    try:
        import comtypes
        regions = ms.AddRegion((comtypes.pointer(pl),))
        if regions:
            return regions[0]
    except Exception:
        pass

    # 中望回退：_REGION 就地转换——多段线被转换为面域，实体总数不变
    doc.SendCommand('(command "_REGION" (handent "%s") "")\n' % pl.Handle)
    for _ in range(50):                       # SendCommand 全异步（坑位 4），必须轮询
        time.sleep(0.2)
        try:
            ent = ms.Item(ms.Count - 1)
            if ent.ObjectName == "AcDbRegion":
                return ent                    # ⚠️ 总数不变，只能判实体类型，不能判 Count 变化
        except Exception:
            pass
    raise RuntimeError("Polyline → Region 转换失败：检查截面是否封闭、多段线是否最后添加")


def bbox_center(solid):
    """实体包围盒中心。核验位置/朝向一律用它，不依赖截图（坑位 5/6）。"""
    lo, hi = solid.GetBoundingBox()
    return [(lo[i] + hi[i]) / 2 for i in range(3)]


def place_solid(solid, target_center):
    """按包围盒中心把实体精准移动到目标中心。
    坑位 2：AddCylinder 等体素轴向恒 +Z 且中心点语义不可靠，定向后统一用本函数归位。"""
    import array
    cur = bbox_center(solid)
    solid.Move(array.array("d", cur), array.array("d", list(target_center)))


def safe_delete(ent):
    """删除可能已被布尔运算吞并的实体：中望残留要删，AutoCAD 已吞并则忽略报错。"""
    try:
        ent.Delete()
    except Exception:
        pass
...
def cleanup_layer_zero(doc):
    """收尾清理：删掉默认层 "0" 上残留的 Region 与辅助曲线（坑位 3）。
    ⚠️ 三维实体(3DSolid)不删；删除前先打印清单，先验后删（坑位 7）。"""
    ms = doc.ModelSpace
    pending = []
    for i in range(ms.Count):
        try:
            ent = ms.Item(i)
            if ent.Layer == "0" and ent.ObjectName in (
                "AcDbRegion", "AcDbLine", "AcDb2dPolyline", "AcDbPolyline",
            ):
                pending.append(ent)
        except Exception:
            continue
    print(f"[cleanup] 将删除默认层残留实体 {len(pending)} 个：", [e.ObjectName for e in pending])
    for ent in pending:
        safe_delete(ent)
```
...
5. **坐标系第一天定死 Z-up**：CAD 惯例 Z 轴向上，后期整体 Rotate3D 转置属于不必要返工（坑位清单第 8 条）
6. **大量实体分段刷新**：数百次 COM 调用时每 30~50 个实体 `Regen(1)` 一次并打印进度，防超时假死（坑位清单第 9 条）
7. **收尾三件事**：`cleanup_layer_zero(doc)` 清残留 → `doc.Regen(1)` → `doc.SaveAs` 落盘，交付前抽查包围盒用数字说话（坑位清单第 10 条）
...
# ── 第 1 步：底板（圆柱基体）──────────────────
# 坑位 2：圆柱轴向恒为 +Z；非 Z 向须 Rotate3D 定向，精准就位用 place_solid() 按包围盒归位
...
safe_delete(boss)   # 坑位 3：中望布尔后工具体残留必须删（AutoCAD 已吞并，报错自动忽略）
doc.Regen(1)
...
# 收尾三件事（坑位 10）：清残留 → Regen → 落盘
cleanup_layer_zero(doc)
doc.Regen(1)
# doc.SaveAs(dwg_path)   # 按实际交付路径落盘
...
# Polyline → Region：一律用辅助函数（坑位 1：中望裸调 ms.AddRegion 必失败）
region = polyline_to_region(doc, ms, profile)

# 拉伸高度 20，锥度 0
solid = ms.AddExtrudedSolid(region, 20.0, 0.0)
safe_delete(profile)   # AutoCAD 上 AddRegion 不吞原曲线；中望走 _REGION 已就地转换，报错自动忽略
safe_delete(region)    # 坑位 3：拉伸用过的面域同样残留，必须手动清理
doc.Regen(1)
# → 回复："L 型支架拉伸完成"
```
...
| 拉伸 / 旋转 | `AddExtrudedSolid`, `AddRevolvedSolid` | 签名一致（用过的面域残留须手动删，见差异表与坑位 3） |
...
| **`AddRegion` 封送** | 接受 COM 对象数组 | **comtypes 传对象数组必失败** | 一律走 `polyline_to_region()`（坑位 1） |
| **布尔运算残留** | 工具体被吞并 | **工具体残留在图中（默认层）** | 每次 `Boolean` 后 `safe_delete(工具体)`，收尾 `cleanup_layer_zero()`（坑位 3） |
| **`AddCylinder` 就位** | 中心点较可靠 | 轴向恒 +Z 且中心点语义不可靠 | `Rotate3D` 定向后用 `place_solid()` 按包围盒中心归位（坑位 2） |
| **`SendCommand` 异步** | 命令通过消息队列异步执行 | 同为异步，完成时机更不可假设 | **发完必须 sleep / 轮询结果**；疑似挂起连发两个 `chr(27)`（ESC）再重试（坑位 4） |
| **COM 事件回调** | 支持 `ObjectModified` 等事件监听 | 部分事件可能不支持 | 本 skill 不使用事件回调，无影响 |

### ⚠️ ZWCAD COM 实战坑位清单（真实项目实测，强制遵守）

以下 10 条全部来自实战反复验证，违反任何一条都会返工。**第 4/5/7/8/9/10 条对所有兼容 CAD（含 AutoCAD）同样适用。** 配套辅助函数见上文「COM 3D 辅助函数」。

1. **AddRegion 数组封送失败**：ZWCAD 的 COM `AddRegion` 不接受 comtypes 直接传入的对象数组。可靠做法：先画封闭多段线，再用命令行转换——`doc.SendCommand('(command "_REGION" (handent "%s") "")\n' % pl.Handle)`，然后轮询 `ModelSpace.Item(Count-1)` 直到它是 Region。注意：多段线是被**转换**为面域的，实体总数不变，**不能用 Count 变化判断是否成功，要判实体类型**（`ObjectName == 'AcDbRegion'`）。已封装进 `polyline_to_region()`。
2. **AddCylinder 轴向固定 +Z**：任何非 Z 向圆柱必须先 `Rotate3D`，且中心点语义不可靠——创建后读 `GetBoundingBox()`，按包围盒中心 `Move` 到目标位置（`place_solid()`），一劳永逸。
3. **布尔运算不删除工具体**：ZWCAD 的 `Boolean` 执行后工具体仍残留在图中（默认层）。用完必须手动 `Delete()`；同理 `AddExtrudedSolid` / `AddRevolvedSolid` 用过的面域也会残留——收尾统一 `cleanup_layer_zero()` 清理，否则模型里全是"幽灵几何"。
4. **SendCommand 全异步**：发完命令必须 sleep 或轮询结果，不能假设立即生效；命令行疑似挂起时连发两个 `chr(27)`（ESC）再重试。
5. **视口白屏 ≠ 实体丢失**：试用版 / 远程桌面下视口经常不刷新，截图（含 PrintWindow）可能全黑。处理：`doc.Regen(1)` 强制重绘；**实体核验一律以 `GetBoundingBox()` 数值为准，不依赖截图**；必须截图时，先加一个临时实体 + Regen + `ZOOM E` 可强制唤醒渲染。
6. **旋转 / 翻转类实体方向必须数值复核**：`AddRevolvedSolid`、镜像、`Rotate3D` 的方向符与直觉常有出入，创建后用 `Centroid` / `GetBoundingBox` 验证朝向，错了就现场反转重放，不要想当然。
7. **实体删除误伤防控**：按条件批量删除时（如按层 + 尺寸），**先打印将删清单再执行**；删除条件要同时锁图层、实体类型、尺寸区间三个维度，单维度条件极易误删（血泪教训：误删吊架顶板）。
8. **坐标系用 Z-up**：建模第一天就定死 Z 轴向上（CAD 惯例），否则用户切俯视看到的是侧面；后期整体 Rotate3D 转置虽然可行（全实体绕 X 轴 +90°），但属于不必要返工。
9. **大图慢操作要分段反馈**：数百次 COM 调用（如逐节铺链）时，每 30~50 个实体 `doc.Regen(1)` 一次并打印进度，既防超时假死，也让用户看到渐进过程。
10. **收尾三件事**：清理默认层残留 → `doc.Regen(1)` → `doc.SaveAs` 落盘。交付前抽查关键实体的包围盒（方向、位置、数量），**用数字说话**。
