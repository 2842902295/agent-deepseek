...
### Step 6: Save and Register Artifact

1. Save as `<descriptive-name>.excalidraw`
2. 调用 `register_artifact`（`artifact_type="excalidraw"`）登记文件，在回复中插入占位符
3. 在回复末尾告知用户：「点击手动编辑，或继续对话告诉我还有什么需要调整的地方。」
   - **禁止**引导用户去 excalidraw.com 或拖入外部工具——前端已内置编辑能力
