...
## 本容器环境补充（项目本地说明，非官方正文）

本服务运行在 Docker 容器内（无桌面 Chrome、无 Browser Use Cloud 账号），官方「Local Chrome」与「Remote Browsers」两条路不适用，**固定走本地 headless CDP**：

1. 先拉起 headless chromium（幂等：已在 9222 端口监听则直接返回）：

```bash
chromium-debug
```

2. 然后按上文官方用法执行 `browser-use <<'PY' ... PY`，daemon 会自动附着该 chromium（user-data-dir 即 `$HOME/.config/chromium`，DevToolsActivePort 自动被发现）。
...
- **不要执行 `browser-use skill install` / `uv tool install`**（需联网装 uv；镜像已内置与本文档匹配的 CLI 0.13.6）
- **不要执行 `browser-use auth login` / cloud 相关命令**（无 cloud 账号）
- `chromium-debug` 失败时查看 `$HOME/.config/chromium/headless.log`；端口可传参覆盖：`chromium-debug 9223`
