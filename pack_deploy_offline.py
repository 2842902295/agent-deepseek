...
SKIP_DIR_NAMES = {
    ...
    # 桌面版打包产物（pack_desktop.py）：绿色目录 / 安装包 exe / 下载缓存（白名单本就不会打入，此处兜底）
    'desktop_dist',
}
SKIP_DIR_GLOBS = ['*.egg-info']

SKIP_FILE_EXTS = {'.pyc', '.pyo', '.log', '.sqlite3', '.sqlite3-shm', '.sqlite3-wal', '.sqlite3-journal'}
# installer.generated.iss：pack_desktop.py 每次运行渲染的 ISCC 中间产物（deploy/desktop/ 随包分发时跳过）
SKIP_FILE_NAMES = {'tortoise_queries.json', 'tortoise_last.txt', 'installer.generated.iss'}
