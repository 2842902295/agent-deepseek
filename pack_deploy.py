...
EXCLUDE_DIRS = {
    ...
    'data',
    'offline_deploy',
    'desktop_dist',      # 桌面版打包产物（pack_desktop.py）：绿色目录 / 安装包 exe / 下载缓存，数百 MB
    'redis_data',
    '.tmp_images',
    '.extraction_log',
    '.qa_workspace',
...
EXCLUDE_FILES = {
    ...
    '.pdm-python',
    # pack_desktop.py 每次运行渲染的 ISCC 中间产物（.gitignore 同样忽略）
...
def collect_pack_options(brand: str | None = None) -> dict:
    ...
    brand 非空且合法时跳过交互菜单（供 pack_deploy_server.py 等非交互调用）。
    """
    if brand in ('standard', 'generic'):
        print(f'[参数指定] 品牌变体: {brand}，跳过交互菜单')
        return {'vite_brand': brand}
...
def should_exclude(file_path: pathlib.Path, base_path: pathlib.Path) -> bool:
    ...
    # 检查文件模式（顶层 README/LICENSE 不进部署包；但 .agent_skills 是随包分发的
    # 技能资产，其自带的 README/LICENSE 属于技能本体的一部分，必须保留）
    rel_parts = file_path.relative_to(base_path).parts
    in_agent_skills = rel_parts[:2] == ('.agent_workspace', '.agent_skills')
    if not in_agent_skills:
...
def create_deploy_package(brand: str | None = None):
    """创建部署包。brand 指定 'standard'/'generic' 时跳过交互菜单。"""
    base_path = pathlib.Path(__file__).parent

    # 收集打包前的可选覆盖配置（仅作用于 zip 内容）
    options = collect_pack_options(brand)
    ...
    zip_filename = f'deploy_package_{options["vite_brand"]}_{timestamp}.zip'
    ...
    # 按文件修改时间排序：文件名含品牌变体段（deploy_package_standard_…），
    # 按文件名字符串排序在新旧命名格式混排时会排错
    import glob
    all_zips = sorted(glob.glob(str(base_path / "deploy_package_*.zip")), key=os.path.getmtime, reverse=True)
    ...
    # 创建一个必读文件说明
    brand_desc = 'standard（内网，端口 1880）' if options['vite_brand'] == 'standard' else 'generic（外网，端口 80 + 443 HTTPS）'
    readme_content = f"""部署包说明
================

打包时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
品牌变体: {options['vite_brand']} — {brand_desc}
...
if __name__ == '__main__':
    # 支持 --brand standard|generic 非交互指定变体（供 pack_deploy_server.py 调用，
    # 否则会再弹一遍品牌变体交互菜单）
    _brand = None
    if '--brand' in sys.argv:
        _idx = sys.argv.index('--brand')
        if _idx + 1 < len(sys.argv):
            _brand = sys.argv[_idx + 1]
    try:
        create_deploy_package(brand=_brand)
