...
/**
 * docx 预览兼容处理（针对 docx-preview 的「边框缺色 → 表格边框消失」bug）：
 * Word 允许边框元素（w:tblBorders / w:tcBorders / w:pBdr 的子元素 w:top/w:insideH/…）
 * 省略 w:color/w:themeColor（语义默认 auto）；但 @vue-office/docx 内置的 docx-preview
 * 取色落空时把 null 直接拼进边框简写 → "0.5pt solid null" 是非法 CSS → 浏览器丢弃
 * 整条 border 声明 → 表格边框全部消失，表格看起来像「没有回显」。
 * 处理：解包 word/*.xml，给缺色边框元素补 color="auto" 后重打包；未命中 / 解析失败
 * 原样返回兜底（不阻断既有行为）。图片等二进制条目以 STORE 原样写回，避免重压缩开销。
 */
export async function standardizeDocxForPreview(buf: ArrayBuffer): Promise<ArrayBuffer> {
  try {
    const {unzipSync, zipSync, strFromU8, strToU8} = await import('fflate');
    const entries = unzipSync(new Uint8Array(buf));
    let dirty = false;
    const out: Record<string, [Uint8Array, {level: number}]> = {};
    for (const [name, data] of Object.entries(entries)) {
      if (/^word\/[^/]+\.xml$/.test(name)) {
        const xml = strFromU8(data);
        const patched = patchDocxBorderColors(xml);
        if (patched !== xml) {
          dirty = true;
          out[name] = [strToU8(patched), {level: 9}];
          continue;
        }
      }
    ...
    return rezipped.buffer.slice(rezipped.byteOffset, rezipped.byteOffset + rezipped.byteLength);
  } catch {
    return buf;
  }
}
...
/**
 * 给 tblBorders/tcBorders/pBdr 块内缺色的边框子元素补 color="auto"。
 * 纯文本变换：块按开闭标签配对截取，子元素只认自闭合写法（OOXML 边框元素恒为空元素）。
 */
export function patchDocxBorderColors(xml: string): string {
  return xml.replace(
    /<([\w-]+):(tblBorders|tcBorders|pBdr)(\s[^>]*)?>([\s\S]*?)<\/\1:\2>/g,
    (_block, ns: string, tag: string, attrs: string, inner: string) => {
      const fixed = inner.replace(
        /<([\w-]+):(top|left|bottom|right|start|end|insideH|insideV|tl2br|tr2bl)([^>]*)\/>/g,
        (m, childNs: string, side: string, childAttrs: string) =>
          /\s(?:[\w-]+:)?(?:themeColor|color)=/.test(childAttrs) ? m : `<${childNs}:${side} ${childNs}:color="auto"${childAttrs}/>`
      );
      return `<${ns}:${tag}${attrs || ''}>${fixed}</${ns}:${tag}>`;
    }
  );
}
