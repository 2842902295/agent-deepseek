...
import { importSpecificStandardsFromMySQL } from '@/service/api';
...
const STORAGE_KEY = 'mysql_specific_migration_config';
...
async function handleSubmit() {
  ...
  try {
    const { data, error } = await importSpecificStandardsFromMySQL({
...
<template>
  <NDrawer v-model:show="visible" :width="500" placement="right">
    <NDrawerContent title="迁入指定标准数据" closable>
      <NAlert type="info" title="说明" :bordered="false" class="mb-16px">
        此功能将从 MySQL 数据库按标准号列表迁入以下五张表的数据：标准基础信息、标准 PDF 信息、PDF 章节、PDF 表格、PDF 公式。已存在的标准基础信息和 PDF 将按标准号更新，子表记录按 ID 更新；不存在则新增。
      </NAlert>
