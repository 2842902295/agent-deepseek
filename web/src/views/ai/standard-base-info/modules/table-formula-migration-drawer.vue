...
import { importTableFormulaFromMySQL } from '@/service/api';
...
const STORAGE_KEY = 'mysql_table_formula_migration_config';
...
async function handleSubmit() {
  ...
  try {
    const { data, error } = await importTableFormulaFromMySQL(formData.value);
...
<template>
  <NDrawer v-model:show="visible" :width="500" placement="right">
    <NDrawerContent title="迁入表格和公式数据" closable>
      <NAlert type="info" title="说明" :bordered="false" class="mb-16px">
        此功能将从 MySQL 数据库只导入 standard_jgh_pdf_table（PDF 表格表）和 standard_jgh_pdf_formula（PDF 公式表）两张表的数据。
        已存在的记录会被自动跳过，不会被覆盖。
      </NAlert>
