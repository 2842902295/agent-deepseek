 ...
 * 这里按品牌登记扩展节点类型（品牌独有或全品牌共用），画布 / 添加节点菜单 / 右键菜单 / 拖线菜单按注册表自动长出对应项。
...
/** 分镜段卡（segNode，分镜板用：一卡一场戏，场内承载多条分镜）——全品牌启用 */
const SEG_NODE_TYPE: BrandNodeTypeDef = {
  nodeType: 'segNode',
  zh: '分镜段',
...
const BRAND_BOARDS: Record<'standard' | 'generic', BrandBoardDef> = {
  // standard 版：后续 standard 独特卡型在此追加
  standard: {
    nodeTypes: [SEG_NODE_TYPE]
  },
  // generic 版：创意生产向——后续 generic 独特卡型在此追加
  generic: {
    nodeTypes: [SEG_NODE_TYPE]
  }
};
