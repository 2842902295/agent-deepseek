...
/* ── 设计语言与 nian 页对齐：QA 蓝玻璃风 ──
   性能约定（同 nian）：节点数量可多，禁 backdrop-filter / 无限动画，阴影单层柔化。
   卡片体与行级 hover 一律无 transition / 无动画：鼠标扫过板面时每张卡都在进出 hover 态，
   带过渡 = 持续多层阴影动画重绘，节点越多掉帧越狠——hover 样式保留但瞬时切换（2026-08-17） */
.wf-node {
  position: relative;
