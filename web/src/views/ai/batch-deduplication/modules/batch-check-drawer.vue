...
const batchName = ref('');
// 查重模式：fast（嵌入式快速，默认）/ deep（精细）
const mode = ref<'deep' | 'fast'>('fast');
...
<template>
  <NDrawer :show="show" :width="800" @update:show="emit('update:show', $event)">
    <NDrawerContent title="批量相似度分析" :native-scrollbar="false" closable>
        ...
        <NCard title="分析模式" :bordered="false" size="small">
          <NRadioGroup v-model:value="mode">
            <NSpace vertical :size="6">
              <NRadio value="fast">
                <span class="font-medium">快速模式（默认）</span>
                <span class="text-gray-500 text-sm ml-2">
                  纯嵌入式向量召回 + 启发式打标，速度大幅提升、效果略差，适合大批量（&gt; {{ RECOMMEND_FAST_THRESHOLD }} 条）
                </span>
              </NRadio>
              <NRadio value="deep">
                <span class="font-medium">精细模式</span>
                <span class="text-gray-500 text-sm ml-2">
                  Agent 自主多维度召回 + 推理打标，结果更准确，无需配置向量库，适合 ≤ {{ RECOMMEND_FAST_THRESHOLD }} 条
                </span>
              </NRadio>
            </NSpace>
          </NRadioGroup>
