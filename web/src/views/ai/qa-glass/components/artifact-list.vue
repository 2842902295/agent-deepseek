...
import {extractExt, standardizeDocxForPreview, standardizeXlsxForPreview} from '@/utils/attachment';
...
async function openFilePreview(a: AgentArtifact) {
  ...
  try {
    ...
    const ext = extractExt(filePreview.name);
    filePreview.src = ext === 'xlsx' ? await standardizeXlsxForPreview(buf) : ext === 'docx' ? await standardizeDocxForPreview(buf) : buf;
  } catch (e: any) {
...
.md-preview-modal {
  ...
  border-radius: 18px;
  /* 子区块（头栏半透白 / 文件预览 body 纸色底）自带不透明背景，
     不裁切会从圆角四角溢出「淡淡的矩形尖角」 */
  overflow: hidden;
  box-shadow:
