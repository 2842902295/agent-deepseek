...
<template>
  <div ref="wrapper" class="html-render-wrap" :class="{'html-render-wrap--fit': fitActive}">
    <!-- 与应用制作画布（html-board.vue）同款放开策略：modals（alert 不被吞）、fullscreen（全屏预览里
         应用还能请求自身全屏）、pointer-lock（画布游戏）、外链弹出不继承沙箱、媒体自动播放 / 剪贴板写 /
         音视频与定位的「允许询问」。gpu 无需配置：WebGL 默认可用、WebGPU 只看安全上下文 -->
    <iframe
      ref="container"
      class="html-render"
