...
import type {AgentExpert, QuickAction, QuickActionGroup} from '@/service/api/ai';
...
const emit = defineEmits<{
  ...
  confirm: [payload: {expertKeys: string[]; actionIds: number[]}];
}>();

const step = ref<1 | 2>(1);
/** 多选专家：勾选 = 加入「我的专家库」，不再绑定单一会话人设 */
...
/** 第二步按橱窗章节分组展示全部功能（icon 透传分类表图标，「更多能力」合成章无图标则不渲染图标位） */
const groupedActions = computed(() => {
  const byId = new Map(props.actions.map(a => [a.id, a]));
  const list: Array<{cat: string; icon?: string; actions: QuickAction[]}> = [];
  const grouped = new Set<number>();
  for (const g of props.groups) {
    const members = g.actionIds.map(id => byId.get(id)).filter((a): a is QuickAction => Boolean(a));
    if (!members.length) continue;
    list.push({cat: g.name, icon: g.icon, actions: members});
...
const previewExpert = computed(() => {
  if (!previewKey.value || !selectedKeys.value.has(previewKey.value)) return null;
  return props.experts.find(e => e.expertKey === previewKey.value) || null;
});

/** 已选专家绑定技能的并集 → 当前可见 action 的 id（供第二步预勾选，保持橱窗序） */
function recommendedActionIds(): number[] {
  const keys = new Set<string>();
  for (const e of props.experts) {
    if (selectedKeys.value.has(e.expertKey)) e.skillKeys.forEach(k => keys.add(k));
  }
  return props.actions.filter(a => a.skillKey && keys.has(a.skillKey)).map(a => a.id);
}
...
function goStep2() {
  ...
  // 进入第二步：按所选专家并集预勾选其绑定技能
  checked.value = new Set(recommendedActionIds());
  step.value = 2;
}
...
function isRecommended(id: number): boolean {
  return recommendedActionIds().includes(id);
}

function confirm() {
  if (!selectedKeys.value.size || checked.value.size === 0) return;
  submitting.value = true;
  emit('confirm', {
    expertKeys: [...selectedKeys.value],
...
<template>
  <NModal :show="show" :mask-closable="false" :closable="false">
    <div class="obm-card" role="dialog" aria-label="新手引导">
      <div class="obm-glow" aria-hidden="true" />

      <header class="obm-head">
        <span class="obm-mark" aria-hidden="true">
          <SvgIcon :icon="step === 1 ? 'mdi:account-tie-outline' : 'mdi:format-list-checks'" />
        </span>
        <div class="obm-titles">
          <h3 class="obm-title">欢迎使用</h3>
          <p class="obm-sub">
            <template v-if="step === 1">STEP 1 · 挑选你的{{ expertLabel }}（可多选，加入{{ expertLabel }}库）</template>
            <template v-else>STEP 2 · 勾选常用技能</template>
          </p>
        </div>
      </header>
      ...
      <!-- 步骤指示 -->
      <div class="obm-steps">
        <span class="obm-step" :class="{active: step === 1, done: step > 1}">1 选择{{ expertLabel }}</span>
        <span class="obm-step-line" :class="{active: step > 1}" />
        <span class="obm-step" :class="{active: step === 2}">2 勾选技能</span>
      </div>
      ...
      <div v-if="step === 1" class="obm-body">
        <div class="obm-prof-grid">
          <button
            ...
            type="button"
            class="obm-prof"
            :class="{selected: selectedKeys.has(expert.expertKey)}"
            @click="toggleExpert(expert.expertKey)"
          >
            <span class="obm-prof-icon">
              <SvgIcon :icon="expert.icon || 'mdi:account-tie-outline'" />
            </span>
            <span class="obm-prof-name">{{ expert.name }}</span>
            <span class="obm-prof-desc">{{ expert.description || '暂无简介' }}</span>
            <span class="obm-prof-count">{{ expert.skillKeys.length }} 项技能</span>
            <span v-if="selectedKeys.has(expert.expertKey)" class="obm-prof-check" aria-hidden="true">
              <SvgIcon icon="mdi:check" />
            </span>
          </button>
        </div>
        <p v-if="previewExpert?.instructions" class="obm-expert-persona">
          <SvgIcon icon="mdi:script-text-outline" />
          <span>{{ previewExpert.instructions.slice(0, 120) }}{{ previewExpert.instructions.length > 120 ? '…' : '' }}</span>
        </p>
      </div>
      ...
      <div v-else class="obm-body obm-body--actions">
        <div class="obm-actions-tip">
          ...
          <span>已按你选的 {{ selectedKeys.size }} 位{{ expertLabel }}预选，可自行增删</span>
        ...
        <div class="obm-actions-scroll">
          <section v-for="group in groupedActions" :key="group.cat" class="obm-group">
            <h4 class="obm-group-title">
              <span v-if="customIconHtml(group.icon)" class="obm-group-ico" aria-hidden="true" v-html="customIconHtml(group.icon)" />
              {{ group.cat }}
            </h4>
            <div class="obm-action-grid">
              ...
              >
                ...
                <span class="obm-action-icon">
                  <!-- 图标双通道：svg 源码/图片 dataURI 走 v-html，iconify 名走 SvgIcon（与橱窗同款） -->
...
/* 选中专家后展示的人设摘要（引导预览同款：让用户先看清这位专家怎么干活） */
.obm-expert-persona {
  display: flex;
  gap: 7px;
...
/* 分组标题图标：与橱窗 pill 同款分类 svg，弱化成灰阶 */
.obm-group-ico {
  display: inline-flex;
  color: var(--ink-3);
}

.obm-group-ico :deep(svg) {
  width: 14px;
  height: 14px;
  display: block;
}
...
/* v-html 通道（svg 源码 / 图片 dataURI）的尺寸，与橱窗卡片同口径 */
.obm-action-icon :deep(svg) {
  width: 16px;
  height: 16px;
  display: block;
}

.obm-action-icon :deep(img) {
  width: 100%;
  height: 100%;
  object-fit: cover;
}
...
/* ─── Kimi 主题：实色白卡 + 中性灰阶，去玻璃/极光 ─── */
:root[data-qa-theme='ink'] .obm-card {
  --paper: #fbfbfc;
  --surface: #ffffff;
  ...
  box-shadow:
    0 0 0 1px rgba(31, 32, 36, 0.07),
    0 2px 8px rgba(0, 0, 0, 0.05),
    0 24px 60px -24px rgba(0, 0, 0, 0.2);
}

:root[data-qa-theme='ink'] .obm-glow {
  display: none;
}

:root[data-qa-theme='ink'] .obm-head {
  border-bottom-color: rgba(31, 32, 36, 0.07);
}

:root[data-qa-theme='ink'] .obm-prof,
:root[data-qa-theme='ink'] .obm-action {
  border-color: rgba(31, 32, 36, 0.1);
  background: #ffffff;
}

:root[data-qa-theme='ink'] .obm-btn--primary {
  box-shadow: none;
}
</style>
