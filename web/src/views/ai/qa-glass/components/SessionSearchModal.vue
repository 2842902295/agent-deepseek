...
async function runSearch() {
  ...
  try {
    ...
    // 画板会话已融合进问答页：搜索结果照常展示（板徽标标识，点击载入后自动伸出其板面板）
...
<template>
  <NModal :show="show" :mask-closable="true" @update:show="v => !v && close()">
    <div class="ssm-card" role="dialog" aria-label="搜索任务">
      ...
      <div class="ssm-meta">
        ...
        <span class="ssm-meta-text">个任务</span>
      ...
      <div class="ssm-list">
        ...
        <p v-else-if="loaded" class="ssm-empty">
          {{ keyword.trim() || dateRange ? '没有找到匹配的任务' : '还没有任务' }}
        </p>
      </div>
    </div>
  </NModal>
...
:root[data-qa-theme='ink'] .ssm-card {
  --paper: #fbfbfc;
  --surface: #ffffff;
...
:root[data-qa-theme='ink'] .ssm-glow {
  display: none;
}

:root[data-qa-theme='ink'] .ssm-head {
  border-bottom-color: rgba(31, 32, 36, 0.07);
}

:root[data-qa-theme='ink'] .ssm-input-wrap {
  border-color: rgba(31, 32, 36, 0.1);
  background: #ffffff;
}

/* 板徽标：画板会话融合进搜索结果后的类型标识（与 QASidebar 同款语言） */
.ssm-board-badge {
  flex-shrink: 0;
  font-style: normal;
  font-size: 9.5px;
...
.ssm-board-badge.sb-board {
  color: #1e40af;
  background: rgba(37, 99, 235, 0.12);
}
.ssm-board-badge.sb-html {
  color: #0e7490;
