...
import { qaTheme, setQaTheme } from '../qa-theme';
...
<template>
  ...
  >
    ...
    <div class="qa-user-card">
      ...
      <!-- 主题风格切换：极光玻璃（默认）/ Kimi 风，存 localStorage -->
      <div class="qa-user-row qa-user-row--static">
        <span class="qa-user-row-label">
          <SvgIcon icon="ph:palette" class="qa-user-ico" />
          <span>主题风格</span>
        </span>
      </div>
...
.qa-user-row:hover .qa-user-row-hint {
  color: var(--accent);
  opacity: 1;
}
...
.qa-user-theme-seg button:hover {
  color: var(--ink);
}

.qa-user-theme-seg button.on {
...
/* ─── Kimi 主题覆盖：实色白卡 + 中性灰阶，去玻璃/极光 ─── */
:root[data-qa-theme='ink'] .qa-user-card {
  --surface: #ffffff;
  --ink: #17181a;
  --ink-2: #494a50;
  --ink-3: #7e7f86;
...
:root[data-qa-theme='ink'] .qa-user-glow {
  display: none;
}

:root[data-qa-theme='ink'] .qa-user-divider {
  background: rgba(31, 32, 36, 0.07);
}

:root[data-qa-theme='ink'] .qa-user-theme-seg {
  background: #efeff1;
}

:root[data-qa-theme='ink'] .qa-user-theme-seg button.on {
  background: #ffffff;
}
</style>

<!-- 非 scoped：弹层 teleport 到 body。raw 只剥离包装层的背景/圆角，默认 box-shadow 仍会
     在透明包装层上画出直角矩形阴影，挂在圆角卡片四角边上，需清零 -->
<style>
.qa-user-pop.n-popover--raw {
  box-shadow: none;
}
</style>
