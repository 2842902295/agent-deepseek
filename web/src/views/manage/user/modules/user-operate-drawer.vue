...
type RuleKey = Extract<keyof Api.SystemManage.UserUpdateParams, 'userName' | 'password' | 'statusType'>;

const rules = computed<Record<RuleKey, App.Global.FormRule[]>>(() => ({
  userName: [defaultRequiredRule],
  // 新增时密码必填且须满足强度校验；编辑时留空表示不修改，填了则同样校验强度
  password:
    props.operateType === 'add'
      ? [defaultRequiredRule, patternRules.pwd]
      : [
          { required: false, trigger: ['blur', 'input'] },
          {
            validator: (_rule: unknown, value: string) => !value || REG_PWD.test(value),
...
async function handleSubmit() {
  ...
  if (props.operateType === 'add') {
    ...
    if (!payload.password) {
      delete (payload as Partial<Model>).password;
    }
