...
import {downloadBlob, downloadText, extractExt, getOfficeKind, isCsvFile, isHtmlFile, isImageFile, isMarkdownFile, isOfficePreviewable, isVideoFile, sanitizeFilename, standardizeDocxForPreview, standardizeXlsxForPreview, type OfficeKind} from '@/utils/attachment';
  ...
  async att => {
    ...
    if (isMarkdownFile(att.name)) {
      ...
      try {
        ...
        if (props.att !== att) return;
        // ClosedXML 式 xlsx（inlineStr + x: 命名空间前缀 → vue-office 解析空白）经 SheetJS 重写为标准格式；
        // docx 边框缺色（docx-preview 拼出非法 CSS 丢边框）补 color="auto"；常规文件原样直出
        const ext = extractExt(att.name);
        office.src = ext === 'xlsx' ? await standardizeXlsxForPreview(buf) : ext === 'docx' ? await standardizeDocxForPreview(buf) : buf;
        if (props.att !== att) return;
      } catch (e: any) {
