...
export function getNaiveTheme(
  ...
  const theme: GlobalThemeOverrides = {
    ...
    /**
     * 全局消息提示：极简白卡方案（样张 message-toast-preview.html 的 B）
     *
     * 这里只放 naive 原生支持的尺寸类变量；结构性效果（图标语义色底块）
     * 与需要分明暗的描边、阴影在 styles/css/naive-overlay.css。
     * 语义色不在此覆盖 —— 沿用 common 的 success/error/warning/info，
     * 保证主题抽屉改配色时消息提示同步跟随。
     */
    Message: {
      borderRadius: '13px',
