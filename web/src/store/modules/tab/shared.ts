...
export function getAllTabs(tabs: App.Global.Tab[], homeTab?: App.Global.Tab) {
  // homeTab 可能为空：当 VITE_ROUTE_HOME 指向的路由对当前角色不可用
  // （后端未返回该菜单、router 中无此路由）时，不渲染固定首页 tab，
  // 避免出现“裸路由名 + 无图标占位图(裂心)”的坏 tab。此时仅返回用户已打开的 tab。
  const sourceTabs = homeTab ? tabs.filter(tab => tab.id !== homeTab.id) : tabs;

  const fixedTabs = sourceTabs.filter(isFixedTab).sort((a, b) => a.fixedIndex! - b.fixedIndex!);

  const remainTabs = sourceTabs.filter(tab => !isFixedTab(tab));

  const allTabs = homeTab ? [homeTab, ...fixedTabs, ...remainTabs] : [...fixedTabs, ...remainTabs];
 ...
 */
export function getDefaultHomeTab(router: Router, homeRouteName: LastLevelRouteKey): App.Global.Tab | undefined {
  const routes = router.getRoutes();
  const homeRoute = routes.find(route => route.name === homeRouteName);

  // 首页路由对当前角色不可用（如角色未授权该菜单，后端未返回，router 中无此路由）时，
  // 返回 undefined，使固定首页 tab 不渲染，避免显示裸路由名与无图标占位图(裂心)。
  if (!homeRoute) {
    return undefined;
  }
