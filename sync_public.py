...
2. 推送前密钥扫描（三重）：
   ...
   - 硬编码口令：password/passwd/secret/api_key/token = "字面量"（放行占位值；
     跳过 .agent_workspace 下原样分发的第三方技能）
   - 动态指纹：读取本项目所有真实 .env 文件里的 KEY/SECRET/PASSWORD/TOKEN 值，
     外加本地清单 .sync_extra_secrets（不进版本库）里登记的口令/账号字面量，
     逐一检查同步内容中是否残留（命中的是「变量名/行号」，不会打印密钥本身）
   任一命中立即中止，绝不推送。
...
EXCLUDE_FILE_PATTERNS = (
    ...
    ".pdm-python", "*.local",
    # 本地额外密钥指纹清单（内容是真口令，绝不入库、绝不同步）
    ".sync_extra_secrets",
    # 根目录一次性调试 / 压测脚本（常硬编码内网地址与口令）
    "bench_*.py", "wire_*.py", "*_probe.py",
)
# 相对路径精确排除（一次性临时脚本 / 内部资料）
EXCLUDE_PATHS = {
    "CLAUDE.md",
    "AGENTS.md",  # 与 CLAUDE.md 同源的内部指令文档（含内网拓扑 / 部署细节）
    "add_menu_temp.py",
    "_fix_migration.py",
    "_write_plan.py",
    "_board_api_test.py",
    ...
    "apipod_extract.txt",
    "apipod_query.md",
    "bench_chapter_fingerprint.py",
    "fix_preview_image_path.sql",
    "generate_visualization.py",
    "pack_deploy_server.py",  # 含各环境服务器 SSH 口令，严禁公开
    "test_artifact_mismatch.html",
    "wire_probe.py",
    "wire_proxy.py",
    # dsh 迁移分支的内部联调脚本（硬编码真实测试账号，且非公开测试套件）
    "tests/dsh_edge_suite.py",
    "tests/dsh_process_replay.py",
    "tests/dsh_stop_midturn_test.py",
}
...
# 硬编码口令特征：键名（可带引号）+ = / : + 字面量值
# 同时覆盖 password="xxx" 与 "password": "xxx" 两种写法；
# 前置负向后行断言排除 'reset-pwd': 'i18n.key' 这类以连字符结尾的键名误报
HARDCODED_CRED_RE = re.compile(
    r"(?<![A-Za-z0-9_-])[\"']?\b(password|passwd|pwd|pass|secret|api_?key|access_?key|token)\b[\"']?"
    r"\s*[=:]\s*([\"'])([^\"'\n]{6,})\2",
    re.IGNORECASE,
)
# 值里出现这些片段即视为占位符 / 非真实凭据，放行（子串匹配，全小写比对）
CRED_PLACEHOLDER_HINTS = (
    "xxxx", "your-", "your_", "change-me", "changeme", "placeholder", "example",
    "<", ">", "{{", "${", "process.env", "import.meta.env", "os.getenv", "os.environ",
    "123456", "password", "passwd", "secret", "token", "api_key", "apikey",
    "none", "null", "true", "false", "test", "demo", "sample", "dummy", "fake",
    "redacted", "todo", "fixme", "xxx",
)
# 硬编码口令特征跳过的路径前缀：原样分发的第三方资产，不适用本项目的书写纪律
# （.agent_workspace = 技能包；excalidraw-assets = 上游打包产物，内含 excalidraw 自己的演示 Firebase 配置）
HARDCODED_CRED_SKIP_PREFIXES = (".agent_workspace/", "web/public/excalidraw-assets/")

# 本地额外密钥指纹清单（每行一个字面量，# 开头为注释）。
# 里面是真口令，因此：不进版本库（.gitignore）、不进公开仓库（EXCLUDE_FILE_PATTERNS）。
EXTRA_SECRETS_FILE = ".sync_extra_secrets"
...
def collect_extra_secrets() -> dict[str, str]:
    """读取本地额外密钥指纹清单 .sync_extra_secrets（文件不存在则返回空）。

    用于登记不在 .env 里的真实凭据：服务器 SSH 口令、测试账号、内网库口令等。
    返回 {指纹描述: 值}，描述形如「.sync_extra_secrets::第3行」，报告只展示描述。
    """
    path = SRC / EXTRA_SECRETS_FILE
    if not path.is_file():
        return {}
    try:
    ...
    for no, line in enumerate(lines, 1):
        ...
        if not value or value.startswith("#") or len(value) < 6:
            continue
        secrets[f"{EXTRA_SECRETS_FILE}::第{no}行"] = value
    return secrets


def _cred_literals(text: str) -> list[str]:
    """提取文本里「口令类字段 = 字面量」的值，过滤掉明显的占位符。"""
    found: list[str] = []
    for m in HARDCODED_CRED_RE.finditer(text):
        value = m.group(3)
        low = value.lower()
        if any(hint in low for hint in CRED_PLACEHOLDER_HINTS):
            continue
        found.append(value)
    return found


def scan_paths(items: Iterable[tuple[str, Path]], env_secrets: dict[str, str]) -> list[str]:
    """对 (相对路径, 文件) 序列做三重扫描，返回命中的「文件 ← 指纹」描述列表。"""
    hits: list[str] = []
    fingerprint_values = list(env_secrets.items())
    todo: list[tuple[str, Path]] = []
    for rel, path in items:
        if rel.startswith(".git/") or path.suffix.lower() in BINARY_EXTS:
            continue
        todo.append((rel, path))
    for i, (rel, path) in enumerate(todo, 1):
        progress(i, len(todo), "已扫描")
        try:
        ...
        if not rel.startswith(HARDCODED_CRED_SKIP_PREFIXES):
            for value in _cred_literals(text):
                hits.append(f"{rel}  ←  硬编码口令字面量（长度 {len(value)}）")
...
def scan_tree(root: Path, env_secrets: dict[str, str]) -> list[str]:
    """扫描目录内全部文本文件（推送前对目标目录的复查）。"""
    candidates = [(p.relative_to(root).as_posix(), p) for p in sorted(root.rglob("*")) if p.is_file()]
    return scan_paths(candidates, env_secrets)
...
def main() -> None:
    ...
    extra_secrets = collect_extra_secrets()
    fingerprints = {**env_secrets, **extra_secrets}
    # 先在源侧对「将要同步的文件」扫一遍，提早暴露问题
    pre_hits = scan_paths(((rel.as_posix(), SRC / rel) for rel in files), fingerprints)
    if pre_hits:
        log("      发现疑似密钥，中止（请清理后重跑）：")
        for h in sorted(set(pre_hits)):
            log(f"      - {h}")
        sys.exit(2)
    log(f"      通过（{len(env_secrets)} 条 .env 指纹 + {len(extra_secrets)} 条本地清单指纹"
        f" + {len(GENERIC_SECRET_PATTERNS)} 类通用特征 + 硬编码口令检查）")
    ...
    if not args.dry_run:
        ...
        post_hits = scan_tree(DST, fingerprints)
        if post_hits:
