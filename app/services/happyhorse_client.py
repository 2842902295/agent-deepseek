...
模型版本不写死：完整模型名 = 块配置的模型基名（agent_model_block 中 VIDEO_HAPPYHORSE
块的 model 字段，system-admin 子 agent 可改，写后热重载生效）+ 任务模式后缀
（-t2v / -i2v / -r2v / -video-edit），见 resolve_model；与 chat / 其它生成块的
「DB 是模型名真相源」管理方式一致。
...
# ── 任务模式与模型拼装 ───────────────────────────────────────────────────────
# 模型版本（如 happyhorse-1.0）不写死在代码：真相源是 agent_model_block 里
# VIDEO_HAPPYHORSE 块的 model 字段（system-admin 子 agent 经 admin_save_record
# 修改，写后自动热重载生效），DEFAULT_MODEL_BASE 只是块 model 留空时的兜底默认。
DEFAULT_MODEL_BASE = "happyhorse-1.0"

MODE_T2V = "t2v"                # 纯文生视频
MODE_I2V = "i2v"                # 首/尾帧图生视频
MODE_R2V = "r2v"                # 多参考图生视频
MODE_VIDEO_EDIT = "video-edit"  # 视频编辑
SUPPORTED_MODES = (MODE_T2V, MODE_I2V, MODE_R2V, MODE_VIDEO_EDIT)


def resolve_model(base: Optional[str], mode: str) -> str:
    """由块配置的模型基名 + 任务模式拼出提交 DashScope 的完整模型名。

    base 来自 VIDEO_HAPPYHORSE 块的 model 字段（DB 真相源），留空落 DEFAULT_MODEL_BASE。
    base 若被写成带模式后缀的全名（如 happyhorse-2.0-t2v）会自动剥掉后缀，
    再按本次任务模式重新拼装，避免拼出 ...-t2v-i2v 这类错误名字。
    """
    m = (mode or "").strip().lower()
    if m not in SUPPORTED_MODES:
        raise HappyHorseError(f"不支持的任务模式：{mode}，可选 {' / '.join(SUPPORTED_MODES)}")
    b = (base or "").strip() or DEFAULT_MODEL_BASE
    for suffix in SUPPORTED_MODES:
        if b.lower().endswith(f"-{suffix}"):
            b = b[: -len(suffix) - 1].strip()
            break
    if not b:
        raise HappyHorseError(f"模型基名解析为空（base={base!r}），请检查 VIDEO_HAPPYHORSE 块的 model 配置")
    return f"{b}-{m}"
...
def _build_payload(
    *,
    ...
    # 模型版本由块配置决定（resolve_model 拼装），这里只校验模式后缀合法
    if not model or not any(model.endswith(f"-{s}") for s in SUPPORTED_MODES):
        raise HappyHorseError(
            f"非法的 HappyHorse 模型名：{model}，应以 {' / '.join('-' + s for s in SUPPORTED_MODES)} 之一结尾"
        )
