...
# 串行化「collection 首建」窗口：空库时并发首调若各自 get_or_create，
# 输家会撞 Table already exists / 元数据写失败 → 偶发 500（见 _ensure_kb_collection_impl 兜底）
_coll_init_lock = threading.Lock()
...
def ensure_kb_collection():
    ...
    if threading.current_thread() is not threading.main_thread():
        ...
        if coll is not None:
            return coll
        # 锁内双检：空库时只放一个线程进去建表，其余等它建完走 has_collection→get 快路径
        with _coll_init_lock:
            if getattr(_tls, "kb_collection", None) is None:
                _tls.kb_collection = _ensure_kb_collection_impl()
            return _tls.kb_collection
...
def _get_or_create_collection_safe(client, *, name: str, schema: Any, embedding_fn: Any):
    """
    带竞态兜底的 get_or_create：跨进程并发首建时输家会撞
    Table already exists / 元数据写失败，赢家已建好，等其可见后改走 get；等不到再抛原错。
    """
    try:
        ...
        logger.warning(f"[seekdb] get_or_create_collection({name}) 竞态冲突（{e}），退避重试 get …")
        for _ in range(10):
            time.sleep(0.5)
            try:
...
def _ensure_kb_collection_impl():
    ...
    coll = _get_or_create_collection_safe(client, name=name, schema=schema, embedding_fn=embedding_fn)
...
def ensure_memstore_collection():
    ...
    with _client_lock:
        ...
        logger.info(f"[seekdb] get_or_create_collection({name}) 开始 …")
        _memstore_collection = _get_or_create_collection_safe(client, name=name, schema=schema, embedding_fn=embedding_fn)
