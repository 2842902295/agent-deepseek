"""
统一技能管理工具集（5 个工具）：skill_read / skill_save / skill_delete / skill_install / skill_list。

平台架构（所有工具的共同前提）：
- **OceanBase 是唯一事实源**：用户技能存 agent_skill（元数据+skill_md）+ agent_skill_file（文件 BLOB+版本）
- 磁盘 `.agent_workspace/.agent_skills/` 只放系统内置技能（只读）；用户 workspace 里的
  `.agent_skills/<key>/` 是运行时物化缓存，随时会被同步逻辑重建/清理，**不是交付物**
- builtin 来源不可删改；skill_key 全局唯一，冲突自动加后缀

设计原则：
- agent 只表达意图（读/写/删/装/列），工具内部处理"动哪张表、碰不碰磁盘"
- 工具内部自捕获异常并返回错误文本，绝不把异常抛回 agent 图（避免整轮对话崩掉）
- 多用户：skill_key 全局唯一，upsert 时检查归属，冲突自动加后缀
"""

from __future__ import annotations

import asyncio
import io
import re
import zipfile
from pathlib import Path
from typing import Annotated, Any, Optional

import httpx
from langchain.tools import tool
from loguru import logger
from tortoise.expressions import Q

from app.models.standard.agent import (
    AgentExpert,
    AgentQuickAction,
    AgentQuickActionExample,
    AgentSkill,
    AgentSkillFile,
    AgentSkillUserPref,
)
from app.services.agent_runtime.skill_pkg import parse_skill_md as _parse_skill_md

_PROJECT_ROOT = Path(__file__).parent.parent.parent.parent
_WORKSPACE_ROOT = _PROJECT_ROOT / ".agent_workspace"
_USERS_ROOT = _WORKSPACE_ROOT / "users"

# 二进制文件扩展名（命中时 is_binary=True，skill_read 不返回内容）
_BINARY_EXTS = frozenset(
    ".png .jpg .jpeg .gif .bmp .ico .webp .svg".split()
    + ".zip .tar .gz .bz2 .7z .rar".split()
    + ".pdf .doc .docx .xls .xlsx .ppt .pptx".split()
    + ".exe .dll .so .dylib .bin .dat".split()
    + ".mp3 .mp4 .avi .mov .wav .flac".split()
    + ".ttf .otf .woff .woff2 .eot".split()
)


def _is_binary_path(path: str) -> bool:
    ext = "." + path.rsplit(".", 1)[-1].lower() if "." in path else ""
    return ext in _BINARY_EXTS


def _validate_key(key: str) -> Optional[str]:
    """校验 skill_key 合法性。返回 None=通过，否则返回错误原因。

    与 @ 召唤正则 `@([^\\s@{}]+)` 对齐：不含空白/@/{}；长度 2~64（DB 上限，
    兼容「技能名_专属code」规范 key）；禁止 . 开头（避免与隐藏目录/相对路径混淆）。
    """
    key = (key or "").strip()
    if not key:
        return "skill_key 不能为空"
    if len(key) < 2 or len(key) > 64:
        return f"skill_key 长度须为 2~64 字，当前 {len(key)}"
    if re.search(r"[\s@{}]", key):
        return "skill_key 不能包含空白字符、@、{、}（会影响 @召唤）"
    if key.startswith("."):
        return "skill_key 不能以 . 开头"
    return None


def _slugify_key_part(s: str) -> str:
    """清洗 key 组成片段：@ 召唤不允许的字符（空白/@/{}）与路径危险字符统一换成 -。"""
    s = re.sub(r"[\\/:*?\"<>|@{}\s]+", "-", (s or "").strip())
    s = re.sub(r"-{2,}", "-", s).strip("-")
    return s.lstrip(".")


# 技能专属 code 字符集：去掉易混淆字符（i/l/o/0/1）
_SKILL_CODE_ALPHABET = "abcdefghjkmnpqrstuvwxyz23456789"


async def _get_or_create_skill_code(uid: int) -> Optional[str]:
    """取用户的技能专属 code；首次使用时自动生成（随机短码、全局唯一、与登录名/手机号完全无关）。"""
    import secrets

    from app.models.system import User

    user = await User.get_or_none(id=uid)
    if user is None:
        return None
    if user.skill_code:
        return user.skill_code
    while True:
        code = "".join(secrets.choice(_SKILL_CODE_ALPHABET) for _ in range(6))
        if not await User.filter(skill_code=code).exists():
            break
    user.skill_code = code
    await user.save(update_fields=["skill_code"])
    return code


async def _build_user_skill_key(uid: Optional[int], name: str) -> str:
    """按规范生成 skill_key：「技能名_专属code」，不允许自由命名。

    专属 code 由系统自动生成（随机短码，与登录名/手机号解耦，防隐私泄露）；
    无登录态时退化为纯名称（名称预算放宽到满 64）。
    """
    code = await _get_or_create_skill_code(uid) if uid is not None else None
    # 有 code 时给「_code」预留 7 字（下划线 + 6 位码）；无登录态纯名称用满 64
    budget = 57 if code else 64
    name_part = _slugify_key_part(name)[:budget] or "skill"
    base = f"{name_part}_{code}" if code else name_part
    return base[:64]


async def _alloc_skill_key(base: str, exclude_id: Optional[int] = None) -> str:
    """key 冲突时自动加后缀（-2、-3…）；exclude_id 排除自身（重算场景）。"""
    final_key = base
    suffix = 2
    while True:
        qs = AgentSkill.filter(skill_key=final_key)
        if exclude_id is not None:
            qs = qs.exclude(id=exclude_id)
        if not await qs.exists():
            return final_key
        sfx = f"-{suffix}"
        # base 最长可达 64 字（无登录态纯名称预算），追加后缀须先截断防超 DB 上限
        final_key = base[: 64 - len(sfx)] + sfx
        suffix += 1


async def apply_key_convention(skill: AgentSkill) -> Optional[str]:
    """按 key 规范重算 skill_key，实现存储 key 与规范的实时一致。

    规范：**已上架（is_enabled=1）= 纯名称**（@ 召唤入口面向全员，不带专属 code）；
    未上架 = 「技能名_专属code」（code 防用户间同名冲突）。
    名称变化 / 上下架后调用：key 有变 → 级联更新文件表/个人偏好/快捷功能/案例/专家绑定
    + 清理旧 workspace 物化目录，返回新 key；未变化返回 None。builtin 不适用。
    """
    if skill.source == "builtin":
        return None
    # 已上架技能 key 不带专属 code；未上架按「技能名_专属code」
    if skill.is_enabled:
        base = _slugify_key_part(skill.name)[:64] or "skill"
    elif skill.user_id is None:
        # 无属主（历史遗留：旧版 is_public 创建的技能 user_id=None，但 key 里带着创建者的专属 code）：
        # 从现有 key 尾部解析 code 与新名称重组（与 _build_user_skill_key 同款预算），
        # 直接重算会把 code 后缀弄丢。解析不到（历史 key 不合规范）→ 保留现有 key
        m = re.search(r"_([abcdefghjkmnpqrstuvwxyz23456789]{6})$", skill.skill_key or "")
        if not m:
            return None
        base = f"{_slugify_key_part(skill.name)[:57]}_{m.group(1)}"
    else:
        base = await _build_user_skill_key(skill.user_id, skill.name)
    new_key = await _alloc_skill_key(base, exclude_id=skill.id)
    if new_key == skill.skill_key:
        return None
    old_key = skill.skill_key
    # 级联所有引用 skill_key 的表：文件 / 个人启停偏好 / 快捷功能 / 案例
    await AgentSkillFile.filter(skill_key=old_key).update(skill_key=new_key)
    await AgentSkillUserPref.filter(skill_key=old_key).update(skill_key=new_key)
    await AgentQuickAction.filter(skill_key=old_key).update(skill_key=new_key)
    await AgentQuickActionExample.filter(skill_key=old_key).update(skill_key=new_key)
    # 专家绑定的技能数组逐个替换（JSON 数组无法按元素精确 UPDATE；专家行数量小，全量扫可接受）
    for exp in await AgentExpert.filter(skill_keys__not_isnull=True):
        keys = list(exp.skill_keys or [])
        if old_key in keys:
            exp.skill_keys = [new_key if k == old_key else k for k in keys]
            await exp.save(update_fields=["skill_keys", "update_time"])
    skill.skill_key = new_key
    await skill.save(update_fields=["skill_key", "update_time"])
    await _purge_workspace_skill(old_key, skill.user_id)
    logger.info(f"技能 key 规范同步：@{old_key} → @{new_key}")
    return new_key


def _fm_field_lines(key: str, v: str) -> list[str]:
    """序列化一个 frontmatter 字段：单行内容 `key: v`；多行内容用块标量 `|`（保留换行，
    与 parse_skill_md 的 PyYAML 解析互为往返）。"""
    if "\n" not in v:
        return [f"{key}: {v}"]
    segs = v.rstrip("\n").split("\n")
    return [f"{key}: |"] + [f"  {seg}" if seg.strip() else "" for seg in segs]


def _extract_pkg_keys(skill_md: str) -> list[str]:
    """从 SKILL.md 的 YAML frontmatter 提取 skills: [k1, k2]。"""
    if not skill_md:
        return []
    m = re.match(r"^---\s*\n(.*?)\n---\s*\n", skill_md, re.S)
    if not m:
        return []
    for line in m.group(1).splitlines():
        line = line.strip()
        if line.startswith("skills:"):
            val = line.split(":", 1)[1].strip()
            if val.startswith("[") and val.endswith("]"):
                return [x.strip().strip("\"'") for x in val[1:-1].split(",") if x.strip()]
    return []


def _build_skill_md(name: str, description: Optional[str], version: Optional[str], body: str) -> str:
    """按 skill 规范生成 SKILL.md：YAML frontmatter + 正文。

    body 已含 frontmatter 时视为完整 SKILL.md，正文原样保留，但 frontmatter 的
    name 强制与字段对齐——堵住「改名只改了字段，SKILL.md 文件里还是旧名」的漂移。
    """
    body = (body or "").strip()
    if body.startswith("---"):
        return _update_frontmatter(body, name=name) + "\n"
    lines = ["---", f"name: {name or 'skill'}"]
    if description:
        lines += _fm_field_lines("description", description)
    lines.append(f"version: {version or '1.0.0'}")
    lines.append("---")
    return "\n".join(lines) + "\n\n" + body + "\n"


def _update_frontmatter(md: str, name: Optional[str] = None, description: Optional[str] = None) -> str:
    """更新 SKILL.md frontmatter 里的 name / description，保持字段与文件一致。

    None = 保留原字段；"" = 删除该字段；其他值 = 覆盖（原 frontmatter 没有的键追加）。
    无 frontmatter 时原样返回。**frontmatter 之后的正文必须原样保留**
    （历史 bug：只拼回 frontmatter 块，正文整个被丢，改个名称就把 SKILL.md 清空了）。

    按逻辑字段整体替换：顶格行（含 ":"）是字段起始，其后的缩进行（块标量
    `description: |` 的续行 / 嵌套字段）归属上一个字段，覆盖/删除时一并处理，
    避免残留旧块标量续行。
    """
    m = re.match(r"^(---\s*\n)(.*?)(\n---\s*\n)", md or "", re.S)
    if not m:
        return md
    head, body, tail = m.group(1), m.group(2), m.group(3)
    rest = md[m.end():]  # frontmatter 之后的正文，原样拼回
    updates = {"name": name, "description": description}
    # 切分逻辑字段：(字段名, [字段所有行])；非字段行（无顶格冒号）挂在上一字段下
    entries: list[tuple[str, list[str]]] = []
    for line in body.splitlines():
        if line[:1] not in (" ", "\t") and ":" in line:
            entries.append((line.split(":", 1)[0].strip(), [line]))
        elif entries:
            entries[-1][1].append(line)
        else:
            entries.append(("", [line]))
    out: list[str] = []
    handled: set[str] = set()
    for key, lines in entries:
        if key in updates:
            handled.add(key)
            v = updates[key]
            if v is None:
                out.extend(lines)
            elif v:
                out.extend(_fm_field_lines(key, v))
            # v == "" → 删除整个字段（含缩进续行）
        else:
            out.extend(lines)
    for key, v in updates.items():
        if key not in handled and v:
            out.extend(_fm_field_lines(key, v))
    return head + "\n".join(out) + tail + rest


async def sync_skill_md_file(skill: AgentSkill) -> None:
    """skill 规范：每个技能必须有一份与 skill_md 字段一致的激活 SKILL.md（字段内容即主文件内容）。"""
    md = _build_skill_md(skill.name, skill.description, skill.version, skill.skill_md)
    content = md.encode("utf-8")
    version = skill.version or "1.0.0"
    # 兼容历史大小写（zip 内可能叫 skill.md）
    rows = await AgentSkillFile.filter(skill_key=skill.skill_key, version=version)
    for f in rows:
        if f.path.lower() == "skill.md":
            if bytes(f.content) != content:
                f.content = content
                f.size = len(content)
                f.is_binary = False
                f.is_active = True
                await f.save()
            return
    await AgentSkillFile.create(
        skill_key=skill.skill_key,
        path="SKILL.md",
        content=content,
        size=len(content),
        is_binary=False,
        version=version,
        is_active=True,
    )


def _can_manage(skill: AgentSkill, uid: Optional[int], role_codes: list[str]) -> bool:
    """当前用户是否有权管理该技能（超管或管理员可管理所有技能）。"""
    if "R_SUPER" in role_codes or "R_ADMIN" in role_codes:
        return True
    if skill.source == "builtin":
        return False
    return skill.user_id == uid


async def _get_role_codes(uid: Optional[int]) -> list[str]:
    if uid is None:
        return []
    from app.models.system import User

    user = await User.get_or_none(id=uid).prefetch_related("by_user_roles")
    if user is None:
        return []
    return [r.role_code for r in user.by_user_roles]


def _builtin_skill_dir(key: str) -> Optional[Path]:
    """磁盘内置技能目录（全局 .agent_skills/ 顶层或 _project/ 下，含 SKILL.md 才算）；不存在返回 None。"""
    if not key or "/" in key or "\\" in key or ".." in key:
        return None
    for d in (_WORKSPACE_ROOT / ".agent_skills" / key, _WORKSPACE_ROOT / ".agent_skills" / "_project" / key):
        if (d / "SKILL.md").is_file():
            return d
    return None


def _iter_builtin_skill_dirs() -> dict[str, Path]:
    """扫全局磁盘内置技能，返回 {key: 目录}；_project 下同名覆盖顶层。"""
    out: dict[str, Path] = {}
    root = _WORKSPACE_ROOT / ".agent_skills"
    if not root.is_dir():
        return out
    for entry in root.iterdir():
        if entry.is_dir() and not entry.name.startswith(".") and entry.name != "_project" and (entry / "SKILL.md").is_file():
            out[entry.name] = entry
    project = root / "_project"
    if project.is_dir():
        for entry in project.iterdir():
            if entry.is_dir() and not entry.name.startswith(".") and (entry / "SKILL.md").is_file():
                out[entry.name] = entry
    return out


# ── workspace 物化（best effort，失败只记日志不影响落库） ────────────────────


def _user_skills_dir(uid: Optional[int]) -> Optional[Path]:
    """当前操作用户的 workspace 技能目录；uid 为空返回 None。"""
    if uid is None:
        return None
    return _USERS_ROOT / str(uid) / ".agent_skills"


async def _materialize_active_files(skill_key: str, uid: Optional[int]) -> None:
    """把刚写入 DB 的激活文件物化到操作用户的 workspace，让 SkillsMiddleware 立即可见。

    与 agent_skill._materialize_from_db 同构（直接写 users/{uid}/.agent_skills/<key>/）。
    60s 节流同步之后也会由它接管维护；这里只是消除空窗。
    """
    target_root = _user_skills_dir(uid)
    if target_root is None:
        return

    def _write() -> None:
        dst = target_root / skill_key
        dst.mkdir(parents=True, exist_ok=True)

    try:
        files = await AgentSkillFile.filter(skill_key=skill_key, is_active=True)
        if not files:
            return
        if not target_root.parent.exists():  # workspace 尚未创建的用户不主动建
            return
        await asyncio.to_thread(_write)
        for f in files:
            target = target_root / skill_key / f.path
            content = bytes(f.content)

            def _write_file(t=target, c=content) -> None:
                t.parent.mkdir(parents=True, exist_ok=True)
                t.write_bytes(c)

            await asyncio.to_thread(_write_file)

        # 清理新激活集之外的残留文件（如升级后新版本删掉的文件），避免 agent 读到过期文件
        active_paths = {f.path for f in files}
        dst = target_root / skill_key

        def _prune() -> None:
            for p in dst.rglob("*"):
                if p.is_file() and p.relative_to(dst).as_posix() not in active_paths:
                    p.unlink(missing_ok=True)
            # 自深向浅回收空目录（顶层技能目录本身保留）
            for d in sorted((d for d in dst.rglob("*") if d.is_dir()), key=lambda x: len(x.parts), reverse=True):
                try:
                    d.rmdir()
                except OSError:
                    pass

        await asyncio.to_thread(_prune)
    except Exception as e:  # noqa: BLE001
        logger.warning(f"物化技能到 workspace 失败（不影响落库）{skill_key}: {e}")


async def _purge_workspace_skill(skill_key: str, uid: Optional[int]) -> None:
    """删除操作用户 workspace 里的技能物化目录（删除技能后的磁盘清理）。"""
    target_root = _user_skills_dir(uid)
    if target_root is None:
        return
    dst = target_root / skill_key
    if not dst.exists():
        return

    import shutil

    try:
        await asyncio.to_thread(shutil.rmtree, dst, True)
    except Exception as e:  # noqa: BLE001
        logger.warning(f"清理 workspace 技能目录失败 {skill_key}: {e}")


# ── 工具 1：skill_read ─────────────────────────────────────────────────────


@tool
async def skill_read(
    key: Annotated[str, "技能 key（@ 召唤用的关键字）"],
    file_path: Annotated[Optional[str], "要读取的文件路径（如 scripts/run.py）；不传则返回技能概览"] = None,
) -> str:
    """读取技能信息（DB 技能 + 磁盘内置技能均可）。不传 file_path 返回概览（元数据+文件列表），传了返回该文件内容。"""
    try:
        skill = await AgentSkill.get_or_none(skill_key=key)

        # 可见性门（隐私）：当前用户不可见的技能（他人未上架）一律按「不存在」处理，
        # 防止任何用户手打 @编辑 @他人key 读取他人未上架技能。不查 is_enabled：
        # 编辑已下架技能是商店管理的正当操作（重新上架流程需要）。
        if skill is not None:
            # 函数级惰性 import：agent_skill 在函数内反向 import 本模块，顶层互引会成环
            from app.api.v1.ai.agent_skill import _is_manager, _listed_or_own, get_current_role_codes

            uid, role_codes, is_super = await get_current_role_codes()
            if not (_listed_or_own(skill, uid) or _is_manager(is_super, role_codes)):
                return f"技能不存在：{key}"
            # 档位白名单门：上架但档位不符的技能同样按「不存在」处理（管理员哨兵恒过）
            from app.api.v1.ai.role_tier import resolve_user_tier_codes, tier_allows

            _tier_codes = await resolve_user_tier_codes(role_codes, _is_manager(is_super, role_codes))
            if not await tier_allows(skill.min_tier_code, skill.user_id, uid, _tier_codes):
                return f"技能不存在：{key}"

        if file_path:
            # 读具体文件：先 DB，未命中再回退磁盘内置技能目录
            f = await AgentSkillFile.get_or_none(skill_key=key, path=file_path, is_active=True)
            if f is not None:
                if f.is_binary:
                    return f"[二进制文件] {file_path}（{f.size} 字节）"
                return f.content.decode("utf-8", errors="replace")
            bd = _builtin_skill_dir(key)
            if bd is not None and ".." not in file_path.split("/"):
                target = (bd / file_path.lstrip("/")).resolve()
                try:
                    target.relative_to(bd.resolve())
                except ValueError:
                    return f"非法路径（越出技能目录）：{file_path}"
                if target.is_file():
                    if _is_binary_path(file_path):
                        return f"[二进制文件] {file_path}（{target.stat().st_size} 字节）"
                    return await asyncio.to_thread(target.read_text, "utf-8", "replace")
            return f"文件不存在：{file_path}（技能 {key}）"

        # 概览：DB 技能
        if skill is not None:
            files = await AgentSkillFile.filter(skill_key=key, is_active=True).order_by("path")
            file_list = "\n".join(f"  {f.path} ({f.size}B)" for f in files) or "  (无文件)"
            parts = [
                f"skill_key: {skill.skill_key}",
                f"name: {skill.name}",
                f"description: {skill.description or ''}",
                f"source: {skill.source}",
                f"version: {skill.version or '-'}",
                f"is_enabled: {bool(skill.is_enabled)}",
                "--- SKILL.md ---",
                skill.skill_md,
                f"--- files ({len(files)}) ---",
                file_list,
            ]
            return "\n".join(parts)

        # 概览：磁盘内置技能（不在 DB，如 skill-creator / office-cli）
        bd = _builtin_skill_dir(key)
        if bd is not None:
            skill_md = await asyncio.to_thread((bd / "SKILL.md").read_text, "utf-8", "replace")
            fm = _parse_skill_md(skill_md)

            def _walk() -> list[str]:
                out: list[str] = []
                for p in sorted(bd.rglob("*")):
                    if p.is_file() and not any(part.startswith(".") for part in p.relative_to(bd).parts):
                        out.append(f"  {p.relative_to(bd).as_posix()} ({p.stat().st_size}B)")
                return out

            fl = await asyncio.to_thread(_walk)
            parts = [
                f"skill_key: {key}",
                f"name: {fm.get('name') or key}",
                f"description: {fm.get('description') or ''}",
                "source: builtin（磁盘内置，未入库；只读，不可 skill_save/skill_delete）",
                "--- SKILL.md ---",
                skill_md,
                f"--- files ({len(fl)}) ---",
                "\n".join(fl) or "  (无文件)",
            ]
            return "\n".join(parts)

        return f"技能不存在：{key}"
    except Exception as e:  # noqa: BLE001
        logger.exception(f"skill_read 失败 key={key}")
        return f"skill_read 执行失败：{type(e).__name__}: {e}"


# ── 工具 2：skill_save ─────────────────────────────────────────────────────

_ICON_MIME = {"png": "png", "jpg": "jpeg", "jpeg": "jpeg", "webp": "webp", "gif": "gif"}


async def _icon_from_workspace_image(rel_path: str, uid: Optional[int]) -> tuple[Optional[str], Optional[str]]:
    """把用户 workspace 内的普通图片读成 data URI 供 icon 直接入库（agent 不经手 base64）。

    返回 (data_uri, error)：成功时 error=None；失败时 data_uri=None。
    路径限定在当前用户 workspace 内，与 skill_install 本地路径同款越权防护。
    不做大小限制——过大的图片由 agent 在调用前自行压缩（提示词有约定），
    避免列表接口携带超大 base64 拖慢技能面板。
    """
    import base64

    if uid is None:
        return None, "图片图标需要登录态"
    ws_root = (_USERS_ROOT / str(uid)).resolve()
    candidate = (ws_root / rel_path.lstrip("./")).resolve()
    try:
        candidate.relative_to(ws_root)
    except ValueError:
        return None, f"非法路径（越出工作区）：{rel_path}"
    if not candidate.is_file():
        return None, f"图片文件不存在：{rel_path}"
    mime = _ICON_MIME.get(candidate.suffix.lower().lstrip("."))
    if mime is None:
        return None, f"暂不支持的图片格式 {candidate.suffix}：仅支持 png/jpg/webp/gif"
    data = await asyncio.to_thread(candidate.read_bytes)
    return f"data:image/{mime};base64,{base64.b64encode(data).decode('ascii')}", None


async def _read_workspace_bytes(rel_path: str, uid: Optional[int]) -> tuple[Optional[bytes], Optional[str]]:
    """读取当前用户 workspace 内文件的原始字节（技能附件文件原样入库用，二进制安全）。

    返回 (bytes, error)：成功时 error=None；失败时 bytes=None。
    路径限定在当前用户 workspace 内，与 skill_install 本地路径 / icon_path 同款越权防护。
    """
    if uid is None:
        return None, "读取 workspace 附件文件需要登录态"
    ws_root = (_USERS_ROOT / str(uid)).resolve()
    candidate = (ws_root / rel_path.lstrip("./")).resolve()
    try:
        candidate.relative_to(ws_root)
    except ValueError:
        return None, f"非法路径（越出工作区）：{rel_path}"
    if not candidate.is_file():
        return None, f"文件不存在：{rel_path}"
    data = await asyncio.to_thread(candidate.read_bytes)
    return data, None


def _notify_skill_sync(uid: Optional[int], *, affects_others: bool = False) -> None:
    """技能写操作后的同步失效钩子（兜底同步周期已拉到 24h，实时到位全靠钩子）。

    对话工具路径一律 force=False：新文件已由 _materialize_active_files 即时物化到
    操作者 workspace，下一条消息只做后台同步清理残留（如改名后的旧 key 目录），
    不值得让流畅对话挨一刀 await。affects_others=True → bump 全局代数，其他用户
    下一条消息后台重同步（已上架技能的可见性/内容变化）。
    """
    try:
        from app.api.v1.ai import qa as _qa

        _qa.notify_skill_changed(uid, affects_others=affects_others, force=False)
    except Exception:  # noqa: BLE001
        pass


@tool
async def skill_save(
    key: Annotated[
        str,
        "技能 key。更新已有技能时传目标技能的现有 key（@ 召唤用的关键字）；"
        "新建时 key 由平台自动按「技能名_专属code」生成，此参数只作名称参考",
    ],
    name: Annotated[Optional[str], "显示名；不传则不改（更新时）或取 key（创建时）"] = None,
    description: Annotated[Optional[str], "简短描述（≤60 字，写明何时触发）"] = None,
    skill_md: Annotated[Optional[str], "SKILL.md 主文件全文（含 YAML frontmatter）；不传则不改"] = None,
    files: Annotated[
        Any,
        "文件列表（JSON 或 list）：[{path, content}] 或 [{path, workspace_path}]。"
        "path 为文件在技能内的位置（如 scripts/run.py、assets/data.zip）；"
        "content 为文本内容（自己编写的文本文件用）；"
        "workspace_path 为用户提供的现有文件的 workspace 相对路径（如 uploads/data.zip），"
        "后端直接读原始字节入库（二进制安全）——zip / 图片 / Office 文档等二进制文件必须走这个参数，"
        "严禁自己读取或转码二进制内容塞进 content。两者同传时以 workspace_path 为准。"
        "传入时会覆盖同路径旧文件；旧版本中不在列表里的文件将转为历史版本。不传则不动文件。",
    ] = None,
    is_enabled: Annotated[
        Optional[bool],
        "商店上/下架：true=上架、false=下架。仅更新已有技能时有效，不传=不改。"
        "上下架仅管理员可操作；普通用户传了会被拒绝。技能默认不上架是常态，"
        "创建/修改技能后不要顺手传本参数，只有管理员用户明确要求上下架时才传",
    ] = None,
    icon: Annotated[
        Optional[str],
        "技能图标：单个 <svg> 元素源码（viewBox='0 0 24 24'、stroke 线条风、自带配色、无 script/外链），"
        "也接受普通图片的 data:image/... base64 URI。新建技能必配；更新时不传=不改，传新值=替换，传空串=清除",
    ] = None,
    icon_path: Annotated[
        Optional[str],
        "用户给的图片要原样当图标时传这个：图片文件的 workspace 相对路径（png/jpg/webp/gif），"
        "后端读文件转 base64 data URI 直接入库，不必自己编码。图片过大请先压缩（长边 ~512px）再传。"
        "传了则优先于 icon 参数",
    ] = None,
    category: Annotated[
        Optional[str],
        "商店分类，从词表选一个：标准工作/办公文档/数据分析/内容创作/视听媒体/开发编程/工具效率/生活娱乐/其他。"
        "新建必配；更新时不传=不改",
    ] = None,
) -> str:
    """创建或更新技能（upsert），写入数据库。key 已存在且有权管理 → 更新；
    无权编辑（他人技能）→ 拒绝并返回提示，由 agent 引导用户换个技能名新建。
    新建时 key 固定由平台按「技能名_专属code」自动生成（不接受自由命名），冲突自动加后缀。
    这是技能持久化的唯一途径——不要把技能写成 workspace 里的文件当交付物。"""
    from app.core.ctx import CTX_USER_ID

    try:
        uid = CTX_USER_ID.get() or None
        role_codes = await _get_role_codes(uid)

        # icon_path：用户图片原样当图标 → 后端读文件转 data URI 入库，优先于 icon
        if icon_path:
            data_uri, err = await _icon_from_workspace_image(icon_path.strip(), uid)
            if err:
                return f"skill_save 图标处理失败：{err}"
            icon = data_uri

        # key 合法性校验
        key = (key or "").strip()
        key_err = _validate_key(key)
        if key_err:
            return f"skill_save 失败：{key_err}"

        # 解析 files 参数
        if isinstance(files, str):
            import json

            try:
                files = json.loads(files)
            except Exception:
                return "files 参数解析失败：不是合法 JSON"
        if files is not None and not isinstance(files, list):
            return f"files 必须是 list，实际：{type(files).__name__}"

        # ── upsert 逻辑 ──
        existing = await AgentSkill.get_or_none(skill_key=key)
        final_key = key

        if existing:
            # builtin 全量保护：任何写入（含仅 files）都拦截
            if existing.source == "builtin":
                return f"技能 {key} 是内置（builtin），不可编辑"
            if not _can_manage(existing, uid, role_codes):
                # 无权编辑他人技能：直接拒绝并给出正确提示，不做任何自动转换
                return (
                    f"无权编辑技能 @{key}：仅技能创建者或超管可修改，原技能未做任何改动。"
                    f"如用户希望在此基础上修改，请引导其创建一个自己的新技能："
                    f"先用 skill_read 读取 @{key} 的内容（含文件），再用 skill_save 换个技能名保存"
                    f"（key 由平台自动按「技能名_专属code」生成）。"
                )

        _pre_name: Optional[str] = existing.name if existing else None

        if existing:
            # ── 更新 ──
            changed: list[str] = []
            if name is not None and name != existing.name:
                existing.name = name[:64]
                changed.append("name")
            if description is not None:
                existing.description = description[:1000] or None
                changed.append("description")
            if skill_md is not None and skill_md != existing.skill_md:
                existing.skill_md = skill_md
                changed.append("skill_md")
                existing.skill_pkg_keys = _extract_pkg_keys(skill_md) or None
                # SKILL.md 为事实源：frontmatter 里的 name/description 回写字段
                # （与 agent_skill.update_skill 同款），否则改名只改 frontmatter 时
                # 字段/key 不跟随，出现「@key 变了、技能名没变」的漂移
                fm = _parse_skill_md(skill_md)
                if fm.get("name") and fm["name"][:64] != existing.name:
                    existing.name = fm["name"][:64]
                    if "name" not in changed:
                        changed.append("name")
                if fm.get("description") and fm["description"][:1000] != (existing.description or ""):
                    existing.description = fm["description"][:1000]
                    if "description" not in changed:
                        changed.append("description")
            if icon is not None:
                new_icon = icon.strip() or None
                if new_icon != (existing.icon or None):
                    existing.icon = new_icon
                    changed.append("icon")
            if category is not None:
                from app.api.v1.ai.agent_skill import _norm_category

                new_cat = await _norm_category(category)
                if new_cat != (existing.category or None):
                    existing.category = new_cat
                    changed.append("category")
            # ── 上/下架（商店可见性 = is_enabled，仅管理员可改）──
            if is_enabled is not None:
                new_en = 1 if is_enabled else 0
                if new_en != existing.is_enabled:
                    if "R_SUPER" not in role_codes and "R_ADMIN" not in role_codes:
                        return "上下架仅管理员可操作：上架由管理员在亲自验证技能后自行决定，技能未上架不影响你自己使用"
                    existing.is_enabled = new_en
                    changed.append("is_enabled")
            if changed:
                await existing.save()
        else:
            # ── 创建 ──
            if not skill_md and not files:
                return "创建技能至少需要 skill_md 或 files 之一"
            if uid is None:
                return "创建技能需要登录态"
            # key 规范：固定「技能名_专属code」，传入的 key 仅作名称参考，冲突自动加后缀。
            # 名称来源：skill_md frontmatter 的 name 优先（SKILL.md 为事实源，
            # 与安装路径/更新分支一致），其次 name 参数，最后 key 参考
            fm_create = _parse_skill_md(skill_md) if skill_md else {}
            eff_name = fm_create.get("name") or name or key or "skill"
            final_key = await _alloc_skill_key(await _build_user_skill_key(uid, eff_name))
            from app.api.v1.ai.agent_skill import _norm_category

            existing = await AgentSkill.create(
                skill_key=final_key,
                name=eff_name[:64],
                description=(description or "")[:1000] or None,
                skill_md=skill_md or "",
                source="derived",
                user_id=uid,
                is_enabled=0,  # 默认未上架：仅创建者可见，管理员上架后全员可见
                icon=(icon or "").strip() or None,
                category=await _norm_category(category),
            )
            changed = ["created"]
            # key 以此名称生成，作为「是否需要重算 key」的基线（见下方重算分支）
            _pre_name = eff_name[:64]

        # ── 文件写入 ──
        files_written = 0
        skill_md_content: Optional[str] = None
        new_paths: list[str] = []
        if files:
            for item in files:
                if not isinstance(item, dict) or "path" not in item:
                    continue
                fpath = item["path"].strip().lstrip("/")
                if not fpath or ".." in fpath.split("/"):
                    continue
                ws_path = (item.get("workspace_path") or "").strip()
                if ws_path:
                    # workspace 文件引用（zip / 图片等二进制附件）：后端直接读原始字节入库，
                    # agent 不经手内容；读取失败整单报错，避免技能缺文件还静默成功
                    content_bytes, err = await _read_workspace_bytes(ws_path, uid)
                    if err:
                        return f"skill_save 附件读取失败（{fpath}）：{err}"
                else:
                    content_str = item.get("content", "")
                    if isinstance(content_str, str):
                        content_bytes = content_str.encode("utf-8")
                    elif isinstance(content_str, (bytes, bytearray)):
                        content_bytes = bytes(content_str)
                    else:
                        content_bytes = str(content_str).encode("utf-8")
                is_bin = _is_binary_path(fpath)

                # 如果 files 里有 SKILL.md，解析 frontmatter 同步元数据
                if fpath.lower() == "skill.md" and not is_bin:
                    skill_md_content = content_bytes.decode("utf-8", errors="replace")
                    fm = _parse_skill_md(skill_md_content)
                    if fm.get("name"):
                        existing.name = fm["name"][:64]
                    if fm.get("description"):
                        existing.description = fm["description"][:1000]
                    if fm.get("version"):
                        existing.version = fm["version"][:32]

                # upsert 文件行（当前激活版本）
                current_version = existing.version or "1.0.0"
                await AgentSkillFile.update_or_create(
                    skill_key=final_key,
                    path=fpath,
                    version=current_version,
                    defaults={
                        "content": content_bytes,
                        "size": len(content_bytes),
                        "is_binary": is_bin,
                        "is_active": True,
                    },
                )
                new_paths.append(fpath)
                files_written += 1

            # 旧激活文件中不在新列表里的 → 转历史版本（与上传升级语义对齐）
            if new_paths:
                await AgentSkillFile.filter(skill_key=final_key, is_active=True).exclude(path__in=new_paths).update(is_active=False)

            await existing.save()

        # ── skill_md ↔ SKILL.md 文件同步（规范：字段内容即主文件内容）──
        if skill_md_content is not None:
            # 传了 SKILL.md 文件 → 文件为准，回写 skill_md 字段
            if existing.skill_md != skill_md_content:
                existing.skill_md = skill_md_content
                existing.skill_pkg_keys = _extract_pkg_keys(skill_md_content) or None
                await existing.save()
        else:
            # 没传 SKILL.md 文件 → 用 skill_md 字段补一份，保证技能始终有主文件
            await sync_skill_md_file(existing)

        # ── frontmatter ↔ 字段双向对齐：显示名 / @key / SKILL.md name 三处必须一致 ──
        # 「SKILL.md → 字段」回写方向上面已完成（skill_md 参数解析 / files 解析）；
        # 这里以字段为准重写 frontmatter（幂等），补上「只传了 name/description 参数」
        # 的路径——历史 bug：改名只更新了字段与 key，SKILL.md 里 frontmatter 的旧名
        # 原样留存，导致 agent 技能目录仍显示旧名。对齐后回刷 SKILL.md 主文件。
        if existing.skill_md:
            aligned_md = _update_frontmatter(
                existing.skill_md,
                existing.name,
                existing.description if ("description" in changed or "created" in changed) else None,
            )
            if aligned_md != existing.skill_md:
                existing.skill_md = aligned_md
                await existing.save()
                await sync_skill_md_file(existing)

        # ── 名称有变或上下架 → 实时重算 key，保持存储与规范一致 ──
        # （规范：上架=纯名称，未上架=技能名_专属code）
        # 更新分支基线=改名前的 name；创建分支基线=生成 key 用的 eff_name——
        # files 里的 SKILL.md 若把 name 改成不同的值，同样要重算
        # （apply_key_convention 自带无变化短路）
        if existing.name != _pre_name or "is_enabled" in changed:
            new_key = await apply_key_convention(existing)
            if new_key:
                final_key = new_key

        # ── 立即物化到操作用户 workspace（best effort，消除同步空窗）──
        await _materialize_active_files(final_key, uid)
        # 已上架技能的内容/上下架变化影响其他用户 → bump 全局代数
        _notify_skill_sync(uid, affects_others=bool(existing.is_enabled) or "is_enabled" in changed)

        summary = f"技能 @{final_key}"
        if "created" in changed:
            summary += f" 已创建（id={existing.id}）"
        else:
            summary += f" 已更新字段：{', '.join(changed)}" if changed else " 无变更"
        if files_written:
            summary += f"；写入 {files_written} 个文件"
        return summary
    except Exception as e:  # noqa: BLE001
        logger.exception(f"skill_save 失败 key={key}")
        return f"skill_save 执行失败：{type(e).__name__}: {e}"


# ── 工具 3：skill_delete ───────────────────────────────────────────────────


@tool
async def skill_delete(
    key: Annotated[str, "要删除的技能 key"],
) -> str:
    """删除技能（DB 记录 + 所有文件）。builtin 不可删。"""
    from app.core.ctx import CTX_USER_ID

    try:
        uid = CTX_USER_ID.get() or None
        role_codes = await _get_role_codes(uid)

        skill = await AgentSkill.get_or_none(skill_key=key)
        if skill is None:
            return f"技能不存在：{key}"
        if skill.source == "builtin":
            return f"技能 {key} 是内置（builtin），不可删除"
        if not _can_manage(skill, uid, role_codes):
            return f"无权删除技能 {key}（非创建者且非超管）"

        # 删文件
        deleted_files = await AgentSkillFile.filter(skill_key=key).delete()
        # 删主记录
        was_listed = bool(skill.is_enabled)  # 删除前记录上架态（已上架 → 影响其他用户）
        await skill.delete()

        # 清理磁盘：用户 workspace 物化目录 + 全局目录历史残留
        await _purge_workspace_skill(key, uid)
        legacy_dir = _WORKSPACE_ROOT / ".agent_skills" / key
        if legacy_dir.exists():
            import shutil

            try:
                await asyncio.to_thread(shutil.rmtree, legacy_dir, True)
            except Exception:
                pass

        _notify_skill_sync(uid, affects_others=was_listed)
        return f"已删除技能 @{key}（含 {deleted_files} 个文件）"
    except Exception as e:  # noqa: BLE001
        logger.exception(f"skill_delete 失败 key={key}")
        return f"skill_delete 执行失败：{type(e).__name__}: {e}"


# ── 工具 4：skill_install ──────────────────────────────────────────────────


@tool
async def skill_install(
    url: Annotated[
        str,
        "技能源地址：zip 包/SKILL.md 的 http(s) URL；或 workspace 内的相对路径"
        "（如 ./out/my-skill.zip，指向本地打包好的技能 zip 或 SKILL.md）",
    ],
    key: Annotated[Optional[str], "可选：技能名参考（最终 key 由平台按「技能名_专属code」规范自动生成，冲突自动加后缀）"] = None,
) -> str:
    """安装技能（下载/读取 → 解析 → 入库）。支持 zip 包和单 SKILL.md。"""
    from app.core.ctx import CTX_USER_ID

    try:
        uid = CTX_USER_ID.get() or None

        if not url:
            return "url 必填"

        url = url.strip()
        is_remote = url.lower().startswith(("http://", "https://"))

        if is_remote:
            try:
                async with httpx.AsyncClient(timeout=60, follow_redirects=True) as client:
                    resp = await client.get(url)
                    resp.raise_for_status()
                    content_type = resp.headers.get("content-type", "")
                    body = resp.content
            except Exception as e:  # noqa: BLE001
                return f"下载失败：{type(e).__name__}: {e}"
            source_url = url
        else:
            # 本地路径：限定在当前操作用户的 workspace 内，防越权读取
            if uid is None:
                return "本地路径安装需要登录态"
            ws_root = (_USERS_ROOT / str(uid)).resolve()
            candidate = (ws_root / url.lstrip("./")).resolve()
            try:
                candidate.relative_to(ws_root)
            except ValueError:
                return f"非法路径（越出工作区）：{url}"
            if not candidate.is_file():
                return f"本地文件不存在：{url}"
            body = await asyncio.to_thread(candidate.read_bytes)
            content_type = ""
            source_url = f"local://{url.lstrip('./')}"

        # 判断 zip 还是 raw SKILL.md
        is_zip = url.lower().endswith(".zip") or "zip" in content_type
        if is_zip:
            msg, _change = await _install_from_zip(body, source_url, key, uid)
            return msg
        else:
            text = body.decode("utf-8", errors="ignore")
            return await _install_from_md(text, source_url, key, uid)
    except Exception as e:  # noqa: BLE001
        logger.exception(f"skill_install 失败 url={url}")
        return f"skill_install 执行失败：{type(e).__name__}: {e}"


def _parse_zip_skill(zip_bytes: bytes) -> tuple[str, list[tuple[str, bytes]], list[str]]:
    """从 zip 字节流解析出 (skill_md, [(rel_path, content_bytes), ...], 附加提示)。纯函数，不碰 DB。

    兼容性处理：
    - GBK 文件名修复：无 UTF-8 标志位的 zip（常见 Windows 中文压缩包）zipfile 按
      cp437 误解码，这里还原成 GBK，避免中文文件名乱码入库
    - 多个 SKILL.md 时取**层级最浅**者为主入口（不再依赖「第一个条目」的打包顺序，
      套件包根 SKILL.md 顺序写错也选得对）
    - 跳过 __MACOSX / 隐藏文件；主入口前缀之外的文件不混入，计数提示
    - SKILL.md 内容按 UTF-8 → GBK → ignore 解码，并规范化 BOM/前导空白
    """
    notes: list[str] = []
    with zipfile.ZipFile(io.BytesIO(zip_bytes)) as zf:
        infos = zf.infolist()

        def _fix_name(info: zipfile.ZipInfo) -> str:
            name = info.filename.replace("\\", "/")
            # 无 UTF-8 标志位：zipfile 已按 cp437 解码，尝试还原 GBK 中文名
            if not (info.flag_bits & 0x800):
                try:
                    name = name.encode("cp437").decode("gbk")
                except (UnicodeEncodeError, UnicodeDecodeError):
                    pass
            return name

        def _junk(name: str) -> bool:
            return any(p == "__MACOSX" or p.startswith(".") for p in name.split("/") if p)

        entries: list[tuple[zipfile.ZipInfo, str]] = []
        for info in infos:
            name = _fix_name(info)
            if info.is_dir() or name.endswith("/") or _junk(name):
                continue
            entries.append((info, name))

        # SKILL.md 候选（basename 不区分大小写）：层级最浅者为主入口，同层取首个
        cands = [(info, name) for info, name in entries if name.rsplit("/", 1)[-1].lower() == "skill.md"]
        if not cands:
            raise ValueError("zip 内未找到 SKILL.md")
        cands.sort(key=lambda t: t[1].count("/"))
        md_info, skill_md_name = cands[0]
        if len(cands) > 1:
            notes.append(f"检测到 {len(cands)} 个 SKILL.md，仅安装主入口（{skill_md_name}），其余随目录作为套件内部文件")

        # 路径前缀（zip 可能带一层或多层顶级目录包裹）
        prefix = skill_md_name.rsplit("/", 1)[0] + "/" if "/" in skill_md_name else ""

        # SKILL.md 内容解码：UTF-8 → GBK → ignore（容忍 GBK 编码的 SKILL.md）
        raw_md = zf.read(md_info)
        try:
            skill_md = raw_md.decode("utf-8")
        except UnicodeDecodeError:
            try:
                skill_md = raw_md.decode("gbk")
            except UnicodeDecodeError:
                skill_md = raw_md.decode("utf-8", errors="ignore")
        # 规范化 BOM/前导空白：一处治 parse/落库原文/_update_frontmatter 三处
        skill_md = skill_md.lstrip("﻿").lstrip()

        file_list: list[tuple[str, bytes]] = []
        skipped_outside = 0
        for info, name in entries:
            if not prefix:
                rel = name
            elif name.startswith(prefix):
                rel = name[len(prefix):]
            else:
                # 主入口前缀之外的文件（同包其它技能/根级杂散文件）不混入
                skipped_outside += 1
                continue
            if not rel or ".." in rel.split("/"):
                continue
            file_list.append((rel, zf.read(info)))
        if skipped_outside:
            notes.append(f"主目录之外另有 {skipped_outside} 个文件已跳过（只安装主 SKILL.md 所属技能）")
        return skill_md, file_list, notes


async def _install_from_zip(
    zip_bytes: bytes,
    source_url: str,
    suggested_key: Optional[str],
    uid: Optional[int],
    allow_upgrade: bool = False,
    source: str = "curated",
) -> tuple[str, dict]:
    """从 zip 字节流安装：在内存中解压，全部存入 DB。

    返回 (message, change)。change: {action, skillKey, name, oldVersion, newVersion, fileCount}；
    allow_upgrade=True 时，上传人名下已有同名技能（非 builtin）→ 原地升级（旧文件转历史版本），
    否则沿用加后缀新建的行为。source 标记来源（discovered=URL 发现安装 / uploaded=用户上传）。
    """

    try:
        skill_md, file_list, notes = await asyncio.to_thread(_parse_zip_skill, zip_bytes)
    except Exception as e:  # noqa: BLE001
        return f"解压失败：{e}", {"action": "failed", "skillKey": None, "name": None, "oldVersion": None, "newVersion": None, "fileCount": 0}

    from app.core.ctx import CTX_USER_ID

    fm = _parse_skill_md(skill_md)
    fm_name = (fm.get("name") or "").strip()
    # key 规范：固定「技能名_专属code」，suggested_key 仅作名称参考
    base_key = await _build_user_skill_key(CTX_USER_ID.get() or uid, fm.get("name") or suggested_key or "skill")

    # ── 升级分支：同属主 + 同名 → 原地升级 ──
    # 不按 key 识别：key 的「_专属code」后缀会被上架/改名等联动改写，不稳定；
    # 也不校验 zip 文件名：文件命名不可控（改名重压/下载再传都会丢匹配）。
    # 只认两个硬条件：技能属上传人本人，且 frontmatter 名称一致（与落库同口径截断 64）。
    # 无 name 不认领任何既有技能、一律新建——多个无名包共享同一 base_key 会互相覆盖。
    # 同名多条（历史 -2 后缀）取最新一条。
    existing = None
    if allow_upgrade and uid is not None and fm_name:
        existing = await AgentSkill.filter(user_id=uid, name=fm_name[:64]).exclude(source="builtin").order_by("-id").first()
    if existing is not None:
        old_version = existing.version
        new_version = fm.get("version") or existing.version or "1.0.0"
        final_key = existing.skill_key  # 兜底命中时旧 key 可能带后缀，一律以既有 key 为准

        existing.name = (fm.get("name") or final_key)[:64]
        if fm.get("description"):
            existing.description = fm["description"][:1000]
        existing.skill_md = skill_md
        existing.skill_pkg_keys = _extract_pkg_keys(skill_md) or None
        existing.version = new_version
        existing.source_url = source_url
        await existing.save()

        # 旧激活文件转历史版本，再写入新版本文件（同版本重传走 update 防唯一键冲突）
        await AgentSkillFile.filter(skill_key=final_key, is_active=True).update(is_active=False)
        for fpath, content_bytes in file_list:
            await AgentSkillFile.update_or_create(
                skill_key=final_key,
                path=fpath,
                version=new_version,
                defaults={
                    "content": content_bytes,
                    "size": len(content_bytes),
                    "is_binary": _is_binary_path(fpath),
                    "is_active": True,
                },
            )

        await _materialize_active_files(final_key, uid)
        # 升级可能命中已上架技能 → 影响其他用户
        _notify_skill_sync(uid, affects_others=bool(existing.is_enabled))

        change = {
            "action": "upgraded",
            "skillKey": final_key,
            "name": existing.name,
            "oldVersion": old_version,
            "newVersion": new_version,
            "fileCount": len(file_list),
        }
        msg = (
            f"已升级 @{final_key}（{existing.name}）· v{old_version or '?'} → v{new_version}\n"
            f"含 {len(file_list)} 个文件。现在可在对话中 @{final_key} 召唤。"
        )
        if notes:
            msg += "\n提示：" + "；".join(notes)
        return msg, change

    # ── 新建分支：防冲突加后缀 ──
    final_key = await _alloc_skill_key(base_key)

    version = fm.get("version") or "1.0.0"

    # 创建主记录（skill_md 即 SKILL.md 全文；默认未上架，仅创建者可见）
    await AgentSkill.create(
        skill_key=final_key,
        name=(fm.get("name") or final_key)[:64],
        description=(fm.get("description") or "")[:1000] or None,
        skill_md=skill_md,
        version=version,
        source_url=source_url,
        source=source,
        user_id=uid,
        is_enabled=0,
    )

    # 写入文件
    for fpath, content_bytes in file_list:
        await AgentSkillFile.create(
            skill_key=final_key,
            path=fpath,
            content=content_bytes,
            size=len(content_bytes),
            is_binary=_is_binary_path(fpath),
            version=version,
            is_active=True,
        )

    await _materialize_active_files(final_key, uid)
    _notify_skill_sync(uid)  # 新装默认未上架，仅本人可见

    msg = (
        f"已安装 @{final_key}（{fm.get('name') or final_key}）· v{version} · 未上架（仅你可见可 @，不影响使用）\n"
        f"含 {len(file_list)} 个文件。现在可在对话中 @{final_key} 召唤。"
    )
    if notes:
        msg += "\n提示：" + "；".join(notes)
    change = {
        "action": "created",
        "skillKey": final_key,
        "name": fm.get("name") or final_key,
        "oldVersion": None,
        "newVersion": version,
        "fileCount": len(file_list),
    }
    return msg, change


async def _install_from_md(
    text: str,
    source_url: str,
    suggested_key: Optional[str],
    uid: Optional[int],
    source: str = "curated",
) -> str:
    """从单个 SKILL.md 文本安装（无附属文件）。"""
    from app.core.ctx import CTX_USER_ID

    fm = _parse_skill_md(text)
    # key 规范：固定「技能名_专属code」，suggested_key 仅作名称参考
    base_key = await _build_user_skill_key(CTX_USER_ID.get() or uid, fm.get("name") or suggested_key or "skill")
    final_key = await _alloc_skill_key(base_key)

    await AgentSkill.create(
        skill_key=final_key,
        name=(fm.get("name") or final_key)[:64],
        description=(fm.get("description") or "")[:1000] or None,
        skill_md=text,
        source_url=source_url,
        source=source,
        user_id=uid,
        is_enabled=0,  # 默认未上架，仅创建者可见
    )

    # SKILL.md 本身也存一份到文件表（方便物化时重建目录结构）
    content_bytes = text.encode("utf-8")
    await AgentSkillFile.create(
        skill_key=final_key,
        path="SKILL.md",
        content=content_bytes,
        size=len(content_bytes),
        is_binary=False,
        version=fm.get("version") or "1.0.0",
        is_active=True,
    )

    await _materialize_active_files(final_key, uid)
    _notify_skill_sync(uid)  # 新装默认未上架，仅本人可见

    return f"已安装 @{final_key}（{fm.get('name') or final_key}）· 未上架（仅你可见，不影响使用）\n现在可在对话中 @{final_key} 召唤。"


# ── 工具 5：skill_list ─────────────────────────────────────────────────────


@tool
async def skill_list(
    keyword: Annotated[Optional[str], "可选：按 key/名称/描述模糊过滤"] = None,
    include_disabled: Annotated[bool, "true=附带已下架的技能行（带「已下架」标记），上架管理时用；默认只列上架中"] = False,
) -> str:
    """列出当前用户可见的技能（DB 技能 + 磁盘内置技能），用于创建前查重、更新前找 key、了解可用能力。
    默认列「已上架（is_enabled=1）或本人创建（未上架也能看到、可自己 @ 用）」；
    include_disabled=true 再附带他人已下架行（带「已下架」标记，上架管理时用）。"""
    from app.core.ctx import CTX_USER_ID

    try:
        uid = CTX_USER_ID.get() or None
        role_codes = await _get_role_codes(uid)
        is_mgr = "R_SUPER" in role_codes or "R_ADMIN" in role_codes

        # 档位白名单过滤用（管理员哨兵恒过）
        from app.api.v1.ai.role_tier import resolve_user_tier_codes, tier_allows

        _tier_codes = await resolve_user_tier_codes(role_codes, is_mgr)

        if include_disabled:
            qs = AgentSkill.all()
        else:
            # 统一尺子：已上架或本人创建（只查 is_enabled=1 会漏掉本人未上架技能）
            q = Q(is_enabled=1)
            if uid is not None:
                q = q | Q(user_id=uid)
            qs = AgentSkill.filter(q)
        rows = await qs.order_by("id")
        lines: list[str] = []
        seen_keys: set[str] = set()
        kw = (keyword or "").strip().lower()
        for s in rows:
            if s.source == "builtin":
                continue
            # 统一尺子：已上架行全见；未上架行（include_disabled 时）仅管理员/作者本人
            if not is_mgr and not s.is_enabled and s.user_id != uid:
                continue
            # 档位白名单：上架但档位不符的技能不进列表（管理员哨兵恒过）
            if not await tier_allows(s.min_tier_code, s.user_id, uid, _tier_codes):
                continue
            if kw and kw not in f"{s.skill_key} {s.name} {s.description or ''}".lower():
                continue
            owner = "公共" if s.user_id is None else ("我的" if s.user_id == uid else "他人")
            has_files = await AgentSkillFile.filter(skill_key=s.skill_key, is_active=True).exists()
            shelf = "" if s.is_enabled else " | 已下架"
            lines.append(
                f"- @{s.skill_key} | {s.name} | {owner}{shelf} | "
                f"source={s.source} | v{s.version or '-'} | {'含文件' if has_files else '纯prompt'} | "
                f"{(s.description or '')[:60]}"
            )
            seen_keys.add(s.skill_key)

        # 磁盘内置技能（未入库，如 skill-creator / office-cli）：对所有人可见、只读
        def _builtin_lines() -> list[str]:
            out: list[str] = []
            for key, d in _iter_builtin_skill_dirs().items():
                if key in seen_keys:
                    continue
                try:
                    fm = _parse_skill_md((d / "SKILL.md").read_text(encoding="utf-8", errors="replace"))
                except Exception:  # noqa: BLE001
                    fm = {}
                name = fm.get("name") or key
                desc = fm.get("description") or ""
                if kw and kw not in f"{key} {name} {desc}".lower():
                    continue
                out.append(f"- @{key} | {name} | 系统内置 | source=builtin(disk) | 只读 | {desc[:60]}")
            return out

        lines.extend(await asyncio.to_thread(_builtin_lines))

        # 附上当前分类词表（DB 维护）：skill_save 的 category 只认这些取值
        from app.api.v1.ai.agent_skill import _skill_categories

        cats = await _skill_categories()
        cat_footer = f"\n[商店分类词表（skill_save.category 只认以下取值）：{' / '.join(cats) or '空'}]"

        if not lines:
            return f"没有可见的技能{f'（关键词：{keyword}）' if kw else ''}{cat_footer}"
        return f"共 {len(lines)} 个可见技能：\n" + "\n".join(lines) + cat_footer
    except Exception as e:  # noqa: BLE001
        logger.exception("skill_list 失败")
        return f"skill_list 执行失败：{type(e).__name__}: {e}"


# ── 存量规范（启动期一次性） ─────────────────────────────────────────────────


async def normalize_listed_skill_keys() -> int:
    """启动期幂等规范：全部已上架技能按「纯名称」重算 key（去掉历史专属 code 后缀）。

    「官方」分类废除后，「已上架」即意味着纯名称 key。apply_key_convention 自带
    无变化短路——已是纯名称的行不产生任何写入，重复执行安全。返回实际改动的技能数。
    """
    changed = 0
    for s in await AgentSkill.filter(is_enabled=1).order_by("id"):
        if s.source == "builtin":
            continue
        if await apply_key_convention(s):
            changed += 1
    return changed


# ── 导出 ───────────────────────────────────────────────────────────────────

SKILL_TOOLS = [skill_read, skill_save, skill_delete, skill_install, skill_list]

__all__ = ["SKILL_TOOLS"]
