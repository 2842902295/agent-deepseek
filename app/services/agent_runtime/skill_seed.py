...
from app.models.standard.agent import AgentSkill, AgentSkillCategory
...
BUILTIN_SKILLS: list[dict] = [
    {
        ...
        "skill_md": (
            ...
            "再用 skill_save 换个技能名保存为用户自己的新技能（落库 key 由平台自动按「技能名_专属code」生成），"
            "然后把新 @key 告知用户。\n\n"
            "## 注意\n"
...
async def seed_builtin_skills() -> None:
    ...
    # 4. 商店分类词表：表为空才播种默认词表；已有数据则完全交给管理员/system-admin 维护
    await seed_skill_categories()


# 商店分类默认词表（与 agent_skill.py::SKILL_CATEGORIES 同序）；播种后真相源即 DB
_DEFAULT_CATEGORIES = [
    "标准工作",
    "办公文档",
    "数据分析",
...
async def seed_skill_categories() -> None:
    """分类词表首启播种：仅当表为空时整表插入，幂等。"""
    if await AgentSkillCategory.all().count() > 0:
        return
    for i, name in enumerate(_DEFAULT_CATEGORIES):
        await AgentSkillCategory.create(name=name, sort_order=i)
