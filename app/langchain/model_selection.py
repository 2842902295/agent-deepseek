...
- AgentModelBlock 表 = 预设块的**运行时真相源**：chat/image/video/embed 四类的清单、凭据、模型属性
...
- AgentModelConfig 仍只存「每类别选了哪个块」（4 行）
- clear_model_caches 切换后清掉全部缓存层（含 embed / seekdb 单例），使新模型对所有用户立即生效

embed 块注意：切换激活块只改「新请求用哪个向量空间」，既有向量库/知识库不会自动重嵌——
库上记录的是构建时快照（embed_block/embed_dim），与激活块不等即陈旧，需显式重建
（见 app/services/vector_hub 的陈旧语义）。
"""
...
DEFAULT_BLOCKS: dict[str, str] = {"chat": "CHAT_DASHSCOPE", "image": "IMAGE", "video": "VIDEO_APIPOD", "embed": "EMBED_DASHSCOPE"}

SUPPORTED_CATEGORIES: tuple[str, ...] = ("chat", "image", "video", "embed")

# 生成类能力的 provider 白名单（每个对应一个已实现的 client；接新厂商必须先写 client 代码）
VALID_PROVIDERS: dict[str, set[str]] = {
    "image": {"gpt", "apipod", "qwen"},
    "video": {"ark", "openrouter", "apipod", "happyhorse", "wan"},
}
...
_SEED_PRESETS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "thinking": None,
        "context_window": None,
        # 思考强度档位（wire 值白名单，实测 qwen3.8-max 支持 none~high）；
        # reasoning_default 留空 = 默认不开（levels 含 none 取 none）
    ...
    },
    {
        ...
        "thinking": None,
        "context_window": None,
        # ⚠️ 0731 版明确不支持 reasoning_effort（百炼文档除外条款）——不播种档位，
        # 前端不展示强度滑块，走 env CHAT_THINKING=false 旧路由
        "sort_order": 2,
    },
    {
        "block_key": "CHAT_KIMI",
        ...
        "api_key": "",  # 凭据来自 .env（_resolve_seed_presets 补齐，同厂商块共用一个 key）
        "model": "kimi/kimi-k3",
        # Kimi-K3 原生多模态（官方支持图片输入），如实声明（2026-09-03 修正，
        # 与既有库数据同步；漏填会致 read_image 拒载、图片直传走兜底通道）
        "vision_supported": True,
        "thinking": None,
        "context_window": None,
    ...
    },
    {
        ...
        "base_url": None,
        "api_key": "",  # 凭据来自 .env（_resolve_seed_presets 补齐，同厂商块共用一个 key）
        # model 存模型基名（不带任务模式后缀），运行期由 happyhorse_client.resolve_model
        # 拼 -t2v / -i2v / -r2v / -video-edit；升级模型版本改这里即可
    ...
    {
        "block_key": "VIDEO_WAN_PRIME",
        "category": "video",
        "label": "万相 Wan3.0 Prime（高速）",
        "provider": "wan",
        "base_url": None,
        "api_key": "",  # 凭据来自 .env（_resolve_seed_presets 补齐，回退 DASHSCOPE_API_KEY）
    ...
    {
        "block_key": "EMBED_DASHSCOPE",
        "category": "embed",
        "label": "百炼 · text-embedding-v4",
        "provider": None,
        "base_url": "https://dashscope.aliyuncs.com/compatible-mode/v1",
        "api_key": "",  # 凭据来自 .env（_resolve_seed_presets 补齐，兼容旧前缀 EMBED_API_KEY）
    ...
    {
        "block_key": "EMBED_LOCAL",
        "category": "embed",
        "label": "本地 · qwen3-embedding",
        "provider": None,
        "base_url": "http://192.168.8.36:9997/v1",
        "api_key": "",  # 凭据来自 .env（_resolve_seed_presets 补齐，兼容旧前缀 LOCAL_EMBED_API_KEY）
...
# 播种凭据的旧前缀回退映射（兼容 .env 老变量名）：块未配置 {BLOCK}_* 时，
# 依次回退读旧前缀的同名变量。既有 .env 的 EMBED_* / LOCAL_EMBED_* 因此继续生效，
# 语义降级为「播种凭据」（与 CHAT_DASHSCOPE_API_KEY 等同口径）。
_SEED_LEGACY_PREFIX: dict[str, str] = {
    "EMBED_DASHSCOPE": "EMBED",
    "EMBED_LOCAL": "LOCAL_EMBED",
    # 万相两块与百炼平台共用同一个 key：{BLOCK}_API_KEY 未配时回退读 DASHSCOPE_API_KEY
    "VIDEO_WAN": "DASHSCOPE",
    "VIDEO_WAN_PRIME": "DASHSCOPE",
}
...
def _resolve_seed_presets() -> list[dict[str, Any]]:
    ...
    {BLOCK}_BASE_URL/_MODEL/_PROVIDER/_DIMENSION 若显式配置也可覆盖基线（便于新装库指向自建网关）。
    旧前缀回退：{BLOCK}_* 未配置时按 _SEED_LEGACY_PREFIX 回退读老变量名
    （EMBED_DASHSCOPE ← EMBED_*，EMBED_LOCAL ← LOCAL_EMBED_*，
    VIDEO_WAN / VIDEO_WAN_PRIME ← DASHSCOPE_*，百炼平台共用一个 key）。
    既有库不受影响——播种幂等跳过已存在的 block_key，运行期真相源始终是 DB。
    """
    resolved: list[dict[str, Any]] = []
    for blk in _SEED_PRESETS:
        b = dict(blk)
        legacy = _SEED_LEGACY_PREFIX.get(b["block_key"])
        for field, suf in _SEED_ENV_FIELDS:
            v = os.environ.get(f"{b['block_key']}{suf}")
            if not v and legacy:
                v = os.environ.get(f"{legacy}{suf}")
            if v:
                b[field] = int(v) if field == "dimension" else v
        resolved.append(b)
    return resolved


# 物化 / 清理 env 时涉及的字段后缀（chat / image/video / embed 的并集；
# _DIMENSION / _BATCH_SIZE 为 embed 块专用，对应 RoleConfig.dimension / batch_size）
_ENV_SUFFIXES = (
    "_BASE_URL",
    "_API_KEY",
    "_MODEL",
...
def _materialize_blocks(blocks: list[dict[str, Any]]) -> None:
    ...
    for b in blocks:
        ...
        for field, suf in (("context_window", "_CONTEXT_WINDOW"), ("dimension", "_DIMENSION"), ("batch_size", "_BATCH_SIZE")):
            v = b.get(field)
            if v is not None:
                os.environ[f"{key}{suf}"] = str(v)
    ...
    _blocks = [
        {
            ...
            "reasoning_levels": (r.reasoning_levels or "").strip() or None,
            "reasoning_default": (r.reasoning_default or "").strip() or None,
...
def _block_configured(category: str, key: str) -> bool:
    """块定义是否完整可用（chat/embed：base_url+api_key+model；image/video：provider+api_key）。"""
    for b in _blocks:
        if b["block_key"] == key and b["category"] == category:
            if category in ("chat", "embed"):
                return bool(b["base_url"] and b["api_key"] and b["model"])
            return bool(b["provider"] and b["api_key"])
    return False


def available_blocks(category: str) -> list[dict[str, Any]]:
    """返回某类别定义完整的预设块（[{key, label, levels}]，供切换页 / PUT 校验用）。

    levels = 思考强度档位白名单（chat 块专属语义；其余类别恒空列表），
    供前端在模型选项上展示「支持强度调节」徽标。加键向后兼容既有消费方。
    """
    cat = category.lower()
    return [
        {"key": b["block_key"], "label": b["label"], "levels": block_reasoning_levels(b["block_key"])}
        for b in _blocks
        if b["category"] == cat and _block_configured(cat, b["block_key"])
    ]


def block_reasoning_levels(block_key: Optional[str]) -> list[str]:
    """chat 块思考强度档位白名单（wire 值序 = 滑块序；env 物化真相源）。

    未配置 / 块不存在 / 入参为空 → 空列表（调用方语义：不展示强度滑块，
    走 env CHAT_THINKING 旧路由）。
    """
    if not block_key:
        return []
    raw = os.environ.get(f"{block_key}_REASONING_LEVELS") or ""
    return [s.strip() for s in raw.split(",") if s.strip()]


def block_reasoning_default(block_key: Optional[str]) -> Optional[str]:
    """chat 块显式配置的默认思考档位（超管 DB 级覆盖）；未配置 → None。

    None 时调用方按「默认不开」规则取档：levels 含 none → none，否则最低档。
    本函数不校验 default ∈ levels（归 chat_mode.block_default_level 统一裁决）。
    """
    if not block_key:
        return None
    return (os.environ.get(f"{block_key}_REASONING_DEFAULT") or "").strip() or None


def block_label(block_key: Optional[str]) -> Optional[str]:
    """块显示名解析：block_key → label（内存块表，启动/重载时已载入）。

    未命中（块已删 / 未加载 / 入参为空）原样返回入参，调用方无需兜底分支。
    """
    if not block_key:
        return block_key
    for b in _blocks:
        if b["block_key"] == block_key:
            return b["label"] or block_key
    return block_key
...
def validate_block_changes(category: str, final_fields: dict[str, Any]) -> Optional[str]:
    ...
    if cat not in SUPPORTED_CATEGORIES:
        return f"category 只能是 chat / image / video / embed，收到：{category!r}"
    provider = final_fields.get("provider")
    if cat in ("chat", "embed"):
        if provider:
            return f"{cat} 块统一走 OpenAI 兼容协议，provider 必须留空"
        missing = [f for f in ("base_url", "api_key", "model") if not final_fields.get(f)]
        if missing:
            return f"{cat} 块必填字段缺失：{missing}"
    else:
        if not provider:
...
async def load_active_blocks_from_db(seed: bool = True) -> None:
    ...
    # 顶部 import（seekdb 等）可能在 lifespan 之前就触发过 load_role("CHAT")
...
def clear_model_caches() -> None:
    """切换后清掉全部缓存层，使新模型对所有用户立即生效。

    每类独立 try/except + logger.exception：单点 import 失败不会吞掉其余清理。
    embed 块进可切换清单后，④⑤ 两层单例也必须清（否则继续用旧向量空间，
    维度/模型全错且不报错）。
    """
    ...
    # ④ embedding 单例（get_embedding 主单例，缓存了按旧块构造的 provider 实例）
    try:
        import app.langchain.embedding_providers as ep

        ep._instance = None
    except Exception:
        logger.exception("[model_selection] 清 embedding 单例失败")
    # ⑤ seekdb 的 embed 函数实例缓存（AsyncBridgeEmbeddingFunction 把
    #    get_embedding() 的实例缓存在 _embed_fn_singleton，不清则知识库/长期记忆
    #    继续用旧维度的向量空间）
    try:
        import app.services.seekdb.client as skc

        skc._embed_fn_singleton = None
    except Exception:
        logger.exception("[model_selection] 清 seekdb embed 单例失败")


async def get_catalog_with_current() -> dict[str, Any]:
    """返回各类别清单 + 当前激活块（取运行时内存映射，即实际生效值；绝不含 api_key）。"""
    result: dict[str, Any] = {}
