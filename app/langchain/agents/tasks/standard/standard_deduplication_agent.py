...
async def _verify_candidates_exist(
        candidates: List[SimilarStandardOutput],
        pool_id: Optional[int] = None,
        target_no: Optional[str] = None,
) -> List[SimilarStandardOutput]:
    """候选标准号存在性硬校验（防 LLM 幻觉 / 手抄错误）。

    LLM 提交的 JSON 是手抄的，可能含编造的标准号、抄错的编号，甚至把目标标准
    自己当成重复项提交。这里以 standard_base_info 为真相源：
    - 剔除目标标准自身、standard_no 为空的条目；
    - 编号必须在库中真实存在（大小写不敏感）；指定 pool_id 时还必须在池内
      （幻觉编号不受召回环节池过滤的约束，需在此补上）；
    - 命中条目用 DB 的规范 standard_no/cname/use_range 回填，顺带纠正名称抄错。
    """
    import aiomysql
    from app.services.mysql_pool import standard_pool
    ...
    target_key = (target_no or "").strip().upper()

    kept: List[SimilarStandardOutput] = []
    dropped: List[str] = []
    for c in candidates:
        key = (c.standard_no or "").strip().upper()
        if not key:
            dropped.append("(standard_no 为空)")
            continue
        if key == target_key:
            dropped.append(f"{c.standard_no}（目标标准自身）")
            continue
        kept.append(c)

    if not kept:
        if dropped:
            logger.warning(f"[Dedup] 存在性校验剔除 {len(dropped)} 条无效候选：{dropped[:10]}")
        return []

    keys = list({(c.standard_no or "").strip().upper() for c in kept})
    found: Dict[str, Dict] = {}
    try:
        async with standard_pool.acquire() as conn:
            async with conn.cursor(aiomysql.DictCursor) as cur:
                ...
                for i in range(0, len(keys), 500):
                    ...
                    await cur.execute(
                        f"SELECT standard_no, cname, use_range FROM standard_base_info "
                        f"WHERE UPPER(standard_no) IN ({placeholders})",
                        chunk,
                    )
                    for row in await cur.fetchall():
                        found[(row.get("standard_no") or "").strip().upper()] = row
    except Exception as e:
        # 校验自身故障不阻断主流程，仅告警
        logger.error(f"[Dedup] 候选存在性校验查询失败，跳过本次校验: {e}")
        return kept
    ...
    if pool_id is not None and pool_id != -1:
        ...
        if nos:
            allowed = {(n or "").strip().upper() for n in nos}

    verified: List[SimilarStandardOutput] = []
    for c in kept:
        key = (c.standard_no or "").strip().upper()
        row = found.get(key)
        if row is None or (allowed is not None and key not in allowed):
            dropped.append(c.standard_no or "")
            continue
        # 以 DB 为真相源回填规范字段
        c.standard_no = row.get("standard_no") or c.standard_no
        if row.get("cname"):
            c.cname = row.get("cname")
        if row.get("use_range"):
            c.use_range = row.get("use_range")
        verified.append(c)

    if dropped:
        logger.warning(f"[Dedup] 存在性校验剔除 {len(dropped)} 条幻觉/无效候选：{dropped[:10]}")
    return verified
    ...
    async with semaphore:
        for attempt in range(1, _MAX_RETRY + 1):
            ...
            try:
                ...
                # 去重 + 存在性硬校验（防幻觉）+ 同系列规则纠正 + 阈值筛选 + 重算 need_attention
