...
2. 生成类「能力块」（IMAGE / VIDEO，未来 TTS / MUSIC …）——对接异构异步 API，
   各家协议不同，无法套 _build_chat。每个能力独立一组四件套：
       {CAP}_PROVIDER   选哪个实现（qwen / gpt / ark / openrouter / happyhorse / wan …）
...
class RoleConfig:
    ...
    # thinking 三态：None=不发送参数(模型默认) / bool=开/关档 / str=reasoning_effort 档位（见 _parse_thinking）
...
class ProviderConfig:
    ...
    provider: str  # qwen / gpt / ark / openrouter / happyhorse / wan / ...（小写）
    api_key: str
...
def _parse_thinking(raw: Optional[str]) -> Optional[Union[bool, str]]:
    """把 thinking 配置值解析成三态（统一 thinking 控制口径）：

    - 空 / 未设置            → None（不发送任何 thinking 参数，完全交给模型默认）
    - none/minimal/low/...   → str（reasoning_effort 档位，原样透传给上游）
    - true/false/1/0/on/off  → bool（开/关档，由各 provider 用其原生参数翻译）
    不识别的值按 None 处理（不发送），避免把错误参数发给上游导致 400。
    """
    if raw is None:
        return None
    s = raw.strip().lower()
    if not s:
        return None
    if s in _REASONING_EFFORT_LEVELS:
        return s
    if s in _THINKING_TRUE:
        return True
    if s in _THINKING_FALSE:
        return False
    return None


# ── 激活块重定向（供"模型切换"页面用）───────────────────────────────────────
# 内存映射：category（chat/image/video/embed）→ 当前选中的 .env 配置块名。
# load_role("CHAT"/"EMBED") / load_provider("IMAGE"/"VIDEO") 据此把请求重定向到超管选中的块；
# 其余角色/能力不受影响（VISION/LOCAL_EMBED 等仍读自己的块）。
...
def load_role(role: str) -> RoleConfig:
    ...
    # embed 同理：全局统一用一个向量模型，"EMBED" 指向选中的 embed 块（默认 EMBED_DASHSCOPE）。
    # LOCAL_EMBED 不重定向——它是独立角色，仅在旧链路过渡期使用。
    if R == "EMBED":
        R = get_active_block("embed", "EMBED_DASHSCOPE")
