...
def clear_agent_instances() -> None:
    ...
    （块定义 / llm 单例没变），不必清全部 4 层。弹出的实例 best-effort 关闭
    dsh 子进程（防泄漏，见 qa._schedule_agent_close）。
    """
    try:
        from app.api.v1.ai.qa import _agent_cache, _schedule_agent_close

        evicted = list(_agent_cache.values())
        _agent_cache.clear()
