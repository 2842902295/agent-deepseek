...
由 get_video_tools() 按 VIDEO 能力（load_provider("VIDEO")）动态选择暴露：
  - ark / openrouter：火山方舟 Seedance（多模态参考）
  - apipod：APIPod Grok Imagine（纯图生视频 I2V）
  - wan：阿里云百炼万相 3.0（All-in-One：T2V/首尾帧/多参考，原生音频）
  - 其它（happyhorse 等）：阿里云 HappyHorse（T2V/I2V/R2V/Video-Edit）

所有 provider 的具体模型版本一律由当前 VIDEO 预设块的 model 字段决定
（agent_model_block 表，超管 / system-admin 子 agent 可改，工具不接受 LLM 覆盖）；
happyhorse 块存模型基名（如 happyhorse-1.0），运行期自动拼任务模式后缀。
LLM 只会看到一个 generate_video 和 query_video_task；VIDEO 能力未配置时返回空列表。

文件组织：共用辅助 → 各 provider 辅助（Ark / HappyHorse / Wan / APIPod）→ 各 provider 工具 → get_video_tools()。
"""
...
from app.services import wan_video_client as wan
from app.services.agent_runtime.call_context import default_user_workspace, get_agent_call_context
...
# Wan3.0 等待上限 1h：长视频（30s / 1080P）实测可达 20~30 分钟，须留足余量；
# 必须 ≤ dsh-mcp-client 的 toolCallTimeoutMs（见 qa_agent._build_cordis_patch_yml），
# 否则工具还没返回 dsh 侧先报 -32001。超时后走 query_wan_video_task 收尾（可下载/登记/计费）。
_WAN_MAX_WAIT = 3600.0
_WAN_POLL = 15.0
# 本进程内已计费的 wan task_id（计费去重，防 generate + query 各记一次）
_WAN_BILLED_TASKS: set[str] = set()
...
def _resolve_workspace() -> Path:
    ...
    # 兜底：按 uid 推导用户工作区（回合上下文缺失时），避免产物误落项目根工作区
    uid = CTX_USER_ID.get()
    if uid:
        return default_user_workspace(uid)
...
def _wan_format_result(
    ...
    *,
    ...
    if status == "SUCCEEDED":
        ...
        in_dur = usage.get("input_video_duration") or 0
        fps = usage.get("fps")
        cost = wan.estimate_cost(
            model,
            f"{sr}P" if sr else None,
            int(out_dur) + int(in_dur) if out_dur is not None else None,
        )
        detail = f"分辨率：{sr}P · 宽高比：{ratio} · 时长：{out_dur}s"
        if fps:
            detail += f" · {fps}fps"
        lines.append(detail)
        if cost is not None:
...
async def _wan_finalize_success(
    result: dict[str, Any],
    *,
    model: str,
    task_id: str,
    ...
    """Wan3.0 任务 SUCCEEDED 后的统一收尾：计费（按 task_id 去重）+ 下载 + 登记 artifact。

    generate_wan_video 等到了成功、或 query_wan_video_task 补查到成功，都走这里——
    保证「等满超时后稍后再查」这条路也能正常落产物、补上计费。
    """
    # 计费按 task_id 去重：同一任务可能先被 generate 记一次、又被 query 补记一次
    try:
        ...
        total_sec = out_dur + in_dur  # 官方口径：输入视频秒数与输出视频秒数同一单价计费
        unit_key = f"video_sec_{sr}" if sr in (480, 720, 1080) else "video_sec_720"
        if total_sec > 0 and task_id not in _WAN_BILLED_TASKS:
            _WAN_BILLED_TASKS.add(task_id)
            await Billing.record(
                module="video",
                provider="dashscope",
                model=model,
                units={unit_key: total_sec},
                ref_type="wan_video_task",
            )
    except Exception:
        logger.exception("[Billing] Wan3.0 视频计费失败（已忽略）")
    ...
    if usage.get("ratio"):
        desc_parts.append(str(usage["ratio"]))

    return await _finalize_video(
        result=result,
        format_result=lambda r, **kw: _wan_format_result(r, model=model, **kw),
        video_url=result.get("video_url"),
        name=name,
        fallback_name=f"wan-{task_id[:8]}",
...
async def generate_happyhorse_video(
    mode: Annotated[
        str,
        "任务模式：t2v（纯文字）/ i2v（图+文）/ r2v（多张参考图+文）/ video-edit（视频+文）。"
        "具体模型版本自动取平台模型配置，无需关心也不能指定",
    ],
    prompt: Annotated[
        Optional[str],
    ...
    resolution: Annotated[
        str, "720P 或 1080P。默认 720P（¥0.9/s）省钱，定稿再升 1080P（¥1.6/s）"
    ] = "720P",
    duration: Annotated[
    ...
    具体用哪个模型版本由平台模型配置决定（超管维护），无需关心也不能指定。
    ...
    # 模型版本一律由当前 VIDEO 预设块的 model 字段决定（DB 真相源，超管/system-admin 可改），
    # 这里只负责任务模式 → 完整模型名的拼装，LLM 不能覆盖版本
    mode_norm = (mode or "").strip().lower()
    try:
        model = hh.resolve_model(load_provider("VIDEO").model, mode_norm)
    ...
    if mode_norm in (hh.MODE_T2V, hh.MODE_R2V):
        parameters["ratio"] = ratio or "16:9"
    if mode_norm != hh.MODE_VIDEO_EDIT:
...
async def generate_wan_video(
    prompt: Annotated[
        str,
        "文本提示词（必填），详细描述（80~200 字）效果远优于简短描述。"
        "传了 media 时，用「图 1」「视频 1」「音频 1」指代 media 数组中对应类型的第 1 个素材，"
        "「图 2」「视频 2」依次类推（图、视频、音频分别计数）。",
    ],
    media: Annotated[
        Optional[Union[list[dict], str]],
        "媒体素材数组，纯文生视频不传。每项 {type, url}，type 可选："
        "first_frame（首帧，≤1 张）/ last_frame（尾帧，≤1 张）/"
        "reference_image（参考图，≤10 张）/ reference_video（参考视频，≤5 段、总时长 ≤15s）/"
        "reference_audio（参考音频，≤5 段、总时长 ≤15s）。"
        "reference_* 与 first_frame/last_frame 互斥，不能混用。"
        "url 支持 http(s)、oss://、data:base64、工作区相对路径（自动上传）。优先传数组，JSON 字符串也可解析。",
    ] = None,
    resolution: Annotated[
        str,
        "480P / 720P / 1080P。默认 720P 省钱，定稿再升 1080P（高速版 720P ¥0.9/s、1080P ¥1.8/s；"
        "标准版约为高速版 2/3 价格）",
    ] = "720P",
    ratio: Annotated[
        str,
        "宽高比：adaptive（自适应，跟随输入素材比例）/ 16:9 / 4:3 / 1:1 / 3:4 / 9:16",
    ] = "adaptive",
    duration: Annotated[
        int,
        "视频时长（秒）：2~30 的整数，**单段原生最长 30 秒，不是 5 秒**（万相 2.x 旧模型的"
        "5 秒限制不适用于万相 3.0）；-1 = 智能时长（模型按内容自动决定）。"
        "默认 5 秒仅为控制成本，用户要长视频时按需求传大值，不要自我设限。"
        "传了参考视频时，输入视频总时长 + 输出时长不能超过 30 秒",
    ] = 5,
    audio: Annotated[
        Optional[bool],
        "输出是否带原生音频（音画一体生成）。不传默认带声音；开关声音价格相同",
    ] = None,
    prompt_extend: Annotated[
        Optional[bool], "是否让平台智能改写/扩展 prompt（短 prompt 建议开启，会增加耗时）；不传默认开启"
    ] = None,
    watermark: Annotated[bool, "是否加水印"] = False,
    seed: Annotated[Optional[int], "随机种子，固定可复现（0~2147483647）"] = None,
    name: Annotated[
        Optional[str], "产物展示名（含 .mp4 后缀），不传按 prompt 截断生成"
    ] = None,
    ...
    """
    用阿里云百炼万相 3.0（Wan3.0）生成视频，内部轮询最长等待 1 小时。
    All-in-One 模型：纯文字 / 首尾帧 / 多参考图视频音频 全能覆盖，
    **单段原生支持 2~30 秒**（30fps，这是原生 30 秒模型；万相 2.x 时代「单段最长 5 秒」
    的老黄历不适用，用户要 10 秒、20 秒、30 秒都直接传，不要声称只能生成 5 秒），
    支持原生音画一体生成（人物可直接开口说话）。
    具体用哪个模型版本（高速版 / 标准版）由平台模型配置决定（超管维护），无需关心也不能指定。
    成功后自动登记 artifact、前端展示视频卡片——不要在文本里贴 URL，简短说明视频内容即可。

    模式选择：只有文字 → 不传 media；"以这张图为首帧/尾帧精确延续" → media 传
    first_frame / last_frame；给参考素材（人/物/场景/视频/音频）→ media 传 reference_*
    并在 prompt 里用「图 1 / 视频 1 / 音频 1」指代。
    prompt 越具体越好：镜头语言、画面元素、光影氛围、动作节奏、台词内容都写清楚。

    返回 TIMEOUT ≠ 失败：只表示等满上限、任务仍在平台侧排队/生成（长视频 20~30 分钟属正常，
    平台偶发僵死任务也会卡住）。此时把 task_id 告知用户，之后用 query_wan_video_task 查询，
    任务完成后该查询会自动下载视频并登记产物。是否立即重试应交用户决定（重试会产生新任务费用，
    旧任务若最终完成平台侧同样计费），不要自作主张反复重试。
    """
    if not has_capability("VIDEO"):
        return "❌ 视频生成不可用：VIDEO 能力未配置（缺 VIDEO_PROVIDER / VIDEO_API_KEY）"

    cfg = load_provider("VIDEO")
    selected_model = cfg.model or wan.DEFAULT_MODEL
    if selected_model not in wan.SUPPORTED_MODELS:
        return f"❌ 不支持的 Wan3.0 模型：{selected_model}，可选 {' / '.join(wan.SUPPORTED_MODELS)}"

    if resolution not in wan.SUPPORTED_RESOLUTIONS:
        return f"❌ resolution 必须是 {' / '.join(wan.SUPPORTED_RESOLUTIONS)} 之一"
    if ratio not in wan.SUPPORTED_RATIOS:
        return f"❌ ratio 必须是 {' / '.join(wan.SUPPORTED_RATIOS)} 之一"
    ...
    if media_list:
        # 与 HappyHorse 同走 DashScope 素材归一化：远端/data: 直通，本地文件上传换 oss://
        try:
            media_list = await _hh_normalize_media_list(media_list, model=selected_model)
        except DashScopeOssError as e:
            return f"❌ 媒体素材处理失败：{e}"
        except Exception as e:
            logger.exception("wan media 归一化异常")
            return f"❌ 媒体素材处理失败：{e}"

    try:
        submit = await wan.submit_task(
            model=selected_model,
            prompt=prompt,
            media=media_list,
            parameters=parameters,
        )
    except wan.WanVideoError as e:
        return f"❌ 提交失败：{e}"
    except Exception as e:
        logger.exception("generate_wan_video 提交异常")
        return f"❌ 提交失败：{e}"

    task_id = submit.task_id
    logger.info(f"Wan3.0 视频任务已提交 {task_id}（model={selected_model}，{duration}s，{resolution}）")

    try:
        result = await wan.wait_task(
            task_id,
            poll_interval=_WAN_POLL,
            max_wait=_WAN_MAX_WAIT,
        )
    except Exception as e:
        logger.exception(f"Wan3.0 任务 {task_id} 轮询异常")
        return f"❌ 轮询失败（task_id={task_id}）：{e}"

    if result.get("status") != "SUCCEEDED":
        return _wan_format_result(result, model=selected_model)

    return await _wan_finalize_success(result, model=selected_model, task_id=task_id, prompt=prompt, name=name)


@tool
async def query_wan_video_task(
    task_id: Annotated[str, "DashScope 返回的 task_id（24 小时有效）"],
) -> str:
    """
    查询已存在万相 3.0 视频任务的进度或结果。仅在用户明确给出旧 task_id 时使用；
    新生成视频请直接用 generate_wan_video（内部已自动轮询）。

    主要用途：generate_wan_video 返回 TIMEOUT（任务仍在平台排队/生成）后，用户过一阵
    拿 task_id 回来查结果。此时若任务已成功，本工具会自动下载视频、登记产物并补计费，
    与 generate 正常成功完全等价。若仍在处理中，如实告知用户再等等。
    """
    if not has_capability("VIDEO"):
        return "❌ 视频查询不可用：VIDEO 能力未配置（缺 VIDEO_PROVIDER / VIDEO_API_KEY）"
    try:
        result = await wan.query_task(task_id)
    except wan.WanVideoError as e:
        return f"❌ 查询失败：{e}"
    except Exception as e:
        logger.exception("query_wan_video_task 异常")
        return f"❌ 查询失败：{e}"
    cfg = load_provider("VIDEO")
    model = cfg.model or wan.DEFAULT_MODEL
    if result.get("status") == "SUCCEEDED":
        # 任务已成功：走完整收尾（下载 + 登记 artifact + 计费去重），与 generate 成功等价
        return await _wan_finalize_success(result, model=model, task_id=task_id)
    return _wan_format_result(result, model=model)
...
async def generate_ark_video(
    ...
    """
    用火山方舟 Seedance 生成视频（纯文字 / 首尾帧 / 多模态参考均可），内部轮询最长等待 12 分钟。
    具体用哪个模型版本由平台模型配置决定（超管维护），无需关心也不能指定。
    成功后自动登记 artifact、前端展示视频卡片——不要在文本里贴 URL，简短说明视频内容即可。
...
async def generate_apipod_video(
    ...
    """
    用 APIPod Grok Imagine 生成视频（仅图生视频 I2V），内部轮询最长等待 12 分钟。
    具体用哪个模型版本由平台模型配置决定（超管维护），无需关心也不能指定。
    成功后自动登记 artifact、前端展示视频卡片——不要在文本里贴 URL，简短说明视频内容即可。
...
def get_video_tools() -> list[Any]:
    ...
    wan → 阿里万相 3.0；其它值（happyhorse 等）→ 阿里 HappyHorse。
    """
    if not has_capability("VIDEO"):
        return []
    # with_lenient_args：兼容模型把 prompt_extend / watermark / audio 等布尔参数传成字符串（"false"/"否"）
    ...
    if provider in ("ark", "openrouter"):
        return [with_lenient_args(t) for t in [generate_ark_video, query_ark_video_task]]
    if provider == "apipod":
        return [with_lenient_args(t) for t in [generate_apipod_video, query_apipod_video_task]]
    if provider == "wan":
        return [with_lenient_args(t) for t in [generate_wan_video, query_wan_video_task]]
    return [with_lenient_args(t) for t in [generate_happyhorse_video, query_happyhorse_video_task]]
