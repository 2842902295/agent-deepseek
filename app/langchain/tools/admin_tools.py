...
async def _reload_model_blocks_safe(src: str) -> None:
    """模型配置表（agent_model_block / agent_model_config）写后钩子：重载块定义 + 清全部缓存层，立即生效。
...
async def _invalidate_billing_cache_safe(src: str) -> None:
    """agent_pricing 写后钩子：失效 Billing 单价缓存（60s TTL），改价秒级生效。

    与 dashboard.py /pricing/upsert 端点同款处理。failure-safe：失败只告警，
    最迟下个 TTL 周期缓存也会自然刷新。
    """
    try:
        ...
        logger.warning(f"[admin_tools] 计费单价缓存失效失败（{src}）：{e}")


async def _clear_tier_cache_safe(src: str) -> None:
    """agent_role_tier 写后钩子：失效档位定义缓存 + 清按用户缓存的 agent 实例。

    用户档位集合编在 agent cache_key（tier_cache_part）里，失效档位缓存 + 清实例后，
    全员下一条消息即按新档位重建可见面。failure-safe：失败只告警。
    """
    try:
        from app.api.v1.ai.role_tier import invalidate_tier_cache
        from app.langchain.role_model_profile import clear_agent_instances

        invalidate_tier_cache()
        clear_agent_instances()
    except Exception as e:
        logger.warning(f"[admin_tools] 档位缓存失效失败（{src}）：{e}")
...
def _load_registry() -> dict:
    ...
    from app.models.standard.video_task import AgentUsageLog, AgentVideoTask
    ...
    return {
        ...
        "agent_skill_category": AgentSkillCategory,
        ...
        # 连接器/数据集本体 + 用户偏好（96a87aa 曾移出，恢复：上下架/档位/描述需经 agent 配置）。
        # 注意：凭据表 agent_connector_credential **刻意不注册**——超管无法经 agent 查谁配了凭据
        "agent_connector": AgentConnector,
        "agent_connector_user_pref": AgentConnectorUserPref,
        "agent_expert": AgentExpert,
        "agent_expert_user_pref": AgentExpertUserPref,
        ...
        "agent_role_tier": AgentRoleTier,
        "agent_pricing": AgentPricing,
        "agent_usage_log": AgentUsageLog,
        "agent_video_task": AgentVideoTask,
        "agent_skill_user_pref": AgentSkillUserPref,
        "agent_user_credit_quota": AgentUserCreditQuota,
        "nian_daily_feed": NianDailyFeed,
        "nian_feed_run_log": NianFeedRunLog,
    }


def _pk_attr(model) -> str:
    """表的主键字段名。绝大多数表是自增 id；agent_user_credit_quota 是自然主键 user_id。"""
    for fname, f in model._meta.fields_map.items():
...
# 只读表：允许 agent 查询统计，但禁止经 admin_save_record / admin_delete_record 改动。
# - agent_usage_log：计费流水是账务事实（cost_yuan/credits 已按写入时单价快照落账），防污染
# - agent_video_task：状态机由异步任务轮询推进，agent 改写会破坏 RUNNING 任务
# - nian_*：收集管道运行记录，只供排查
# 改价走 agent_pricing（新建记录，自动失效旧价）。
_READONLY_TABLES: set = {"agent_usage_log", "agent_video_task", "nian_daily_feed", "nian_feed_run_log"}
...
def make_admin_tools(user_id: Optional[int]):
    ...
    async def admin_tables() -> str:
        ...
        for tname, model in registry.items():
            rows.append({
                ...
                # 注册表内表默认可写（字段自动推导，除主键/自动时间戳）；只读表例外（见 _READONLY_TABLES）
                "writable": tname not in _READONLY_TABLES,
            })
        return _ok(rows)
    ...
    async def admin_sql(sql: Annotated[str, "要执行的只读 SELECT 语句（只能查 admin_tables 里的表）"]) -> str:
        ...
        agent_usage_log=全部 AI 用量流水（module/provider/model/units_json/cost_yuan/credits/user_id，
        按用户/模型/模块统计用量与花费都查这张，只读）；agent_pricing=单价表（版本化，
        effective_to IS NULL 为当前生效价）。查「哪些模型没配价」：agent_model_block（is_deleted=0）
        与 agent_pricing（生效行）按模型名对比即可。
        其他口径：用户积分余额 = agent_user_credit_quota.quota（无记录=默认配额）减去
        SUM(agent_usage_log.credits)；视频生成排查查 agent_video_task（status/error_code/
        error_message，agent_usage_log.ref_id 在 ref_type='video_task' 时指向它的 id）；
        用户技能启停查 agent_skill_user_pref（is_enabled/is_added，无记录=默认启用且未添加，
        仅本人创建的技能默认已添加）。
        """
    ...
    async def admin_save_record(
        ...
        - agent_model_block：模型预设块定义（加模型 / 改模型配置 / 升级模型版本=改 model 字段，
          写后自动热重载立即生效）。chat / embed 块 provider 留空、base_url/api_key/model 必填（OpenAI 兼容协议）；
          embed 块另有 dimension 字段（向量输出维度，整数）——换不同维度的 embed 块会使既有向量库陈旧，
          页面切换时会提示，加块时 dimension 必须如实填写；
          image/video 块 provider 必填且只能是已实现厂商，其 model 字段是运行期模型名真相源
          （happyhorse 视频块存模型基名如 happyhorse-1.0，运行期自动拼任务模式后缀）；
          block_key 建好后不可改。api_key 回显一律脱敏；**复用已有块的 key（同厂商加新模型）
        ...
        - agent_pricing：模型单价（版本化，provider+model+unit_type+生效区间）。**改价 = 新建一条记录**
          （不传 record_id），后端自动把同条目当前生效行置为失效，秒级生效；effective_from 不传默认
          当前时间。unit_type 常见值：token_in/token_out/token_cached（文本）、video_sec_*（视频）、
          image_*（图片）、mcp_call 等，可先用 admin_sql 查 agent_usage_log.units_json 里实际出现过的键
        - agent_user_credit_quota：用户积分配额（generic 品牌生效）。**user_id 是主键**（record_id 即
          user_id）；新建给 {user_id, quota}，该用户已有配额行时自动转为更新（upsert）。删除配额行 =
          该用户回落默认配额，不是清零
        - 只读表（agent_usage_log / agent_video_task / nian_daily_feed / nian_feed_run_log）禁止写入，
          统计排查请用 admin_sql
        """
        from app.utils.security import get_password_hash

        tname = _normalize_table(table)
        if tname in _READONLY_TABLES:
            return _err(f"拒绝：{tname} 是只读表（账务/运行流水，不可改写）。查询统计请用 admin_sql")
        registry = _load_registry()
        ...
        # 自然主键表（如 agent_user_credit_quota 的 user_id）：新建时主键值在 changes 里给，放开校验
        if record_id is None and pk != "id" and pk not in allowed_fields:
            allowed_fields = allowed_fields + [pk]
        ...
        for k in list(changes):
            ...
            if enum_type is not None:
                ...
                changes[k] = name
        # 时间字段归一（通用）：agent 通常把时间传成 ISO 字符串，Tortoise 需要 datetime 对象
        ...
        for k in list(changes):
            ...
            if v is None or not isinstance(v, str):
                continue
            f = model._meta.fields_map.get(k)
            if f is None or type(f).__name__ != "DatetimeField":
                continue
            try:
                changes[k] = _dt.fromisoformat(v.replace("Z", "+00:00")).replace(tzinfo=None)
            except ValueError:
                return _err(f"{k} 不是合法时间：{v!r}（用 ISO 格式，如 2026-08-13T10:00:00）")
        # agent_user_credit_quota 的 quota 数值校验前置（在自然主键查库之前，非法输入不碰 DB）
        if tname == "agent_user_credit_quota" and "quota" in changes:
            try:
                q = int(str(changes["quota"]))
            except (TypeError, ValueError):
                return _err(f"quota 必须是整数：{changes['quota']!r}")
            if q < 0:
                return _err("quota 不能为负数")
            changes["quota"] = q
        ...
        # min_tier_code 归一（技能/专家/连接器）：可见档位白名单数组，只认 agent_role_tier 词表内 code；
        # 'all'/空串 → NULL（全员可见）；未知 code 报错并列出合法值（与端点同口径）
        if "min_tier_code" in changes and tname in ("agent_skill", "agent_expert", "agent_connector"):
            from app.api.v1.ai.role_tier import norm_tier_codes

            _norm_tier, _tier_err = await norm_tier_codes(changes["min_tier_code"])
            if _tier_err:
                return _err(_tier_err)
            changes["min_tier_code"] = _norm_tier

        if record_id is not None:
            rec = await model.get_or_none(**{pk: record_id})
            if rec is None:
                return _err(f"{tname} 记录不存在：{pk}={record_id}")
        else:
            rec = None
        # 自然主键表（pk != id，如 agent_user_credit_quota 的 user_id）：
        # 新建时主键值在 changes 里；已有同主键记录 → 转为更新该行（upsert 语义，
        # 与 dashboard /credit-quotas 端点的 ON DUPLICATE KEY UPDATE 一致）；更新时禁改主键
        if pk != "id":
            if rec is None:
                pk_val = changes.get(pk)
                if pk_val in (None, ""):
                    return _err(f"新建 {tname} 记录必须提供主键 {pk}")
                try:
                    changes[pk] = int(str(pk_val))
                except (TypeError, ValueError):
                    return _err(f"主键 {pk} 必须是整数：{pk_val!r}")
                existing = await model.get_or_none(**{pk: changes[pk]})
                if existing is not None:
                    rec = existing
                    changes.pop(pk, None)  # 已存在转更新，主键不动
            elif pk in changes:
                return _err(f"主键 {pk} 不可修改；要管理其他用户的记录请新建")
        ...
        if tname == "agent_model_block":
            ...
            if rec is None:
                ...
                if not re.fullmatch(r"(CHAT|IMAGE|VIDEO|EMBED)(_[A-Z0-9]+)?", bk):
                    return _err("block_key 须以 CHAT/IMAGE/VIDEO/EMBED 开头，仅大写字母/数字/下划线，如 CHAT_DEEPSEEK / EMBED_LOCAL")
            ...
            if emsg:
                return _err(emsg)
        if tname == "agent_pricing" and rec is None:
            from decimal import Decimal as _Decimal
            from decimal import InvalidOperation as _InvalidOperation

            missing = [k for k in ("provider", "model", "unit_type", "price_yuan") if changes.get(k) in (None, "")]
            if missing:
                return _err(f"新建单价缺少必填字段：{missing}（unit_type 如 token_in / token_out / video_sec_720）")
            try:
                price = _Decimal(str(changes["price_yuan"]))
            except (_InvalidOperation, TypeError, ValueError):
                return _err(f"price_yuan 不是合法数字：{changes['price_yuan']!r}（每单位价，元）")
            if price < 0:
                return _err("price_yuan 不能为负数")
            changes["price_yuan"] = price
            for sk in ("provider", "model", "unit_type"):
                changes[sk] = str(changes[sk]).strip()
            # 版本化约定：新建默认立即生效（与 dashboard /pricing/upsert 的 NOW(6) 语义一致）
            if not changes.get("effective_from"):
                changes["effective_from"] = _dt.now()
        if tname == "agent_user_credit_quota" and rec is None:
            # 真新建（非 upsert 转更新）：用户必须存在（与 dashboard set_credit_quota 同款校验）
            from app.models.system.admin import User

            uid = changes.get("user_id")
            if not await User.filter(id=uid).exists():
                return _err(f"用户不存在：user_id={uid}（先用 admin_sql 查 users 表确认）")
        ...
        preview: dict = {
            ...
            **({"recordId": getattr(rec, pk)} if rec is not None else {}),
        ...
        try:
            ...
            # 改价 = 版本化插新行：新价落库后把同条目旧的生效行置为失效
            #（与 dashboard.py /pricing/upsert 的「失效旧 + 插新」事务语义一致，
            # 历史 agent_usage_log 按 pricing_snapshot_json 对账，不受影响）
            if tname == "agent_pricing" and preview["mode"] == "create":
                from tortoise import Tortoise

                from app.settings.config import settings

                conn = Tortoise.get_connection(next(iter(settings.TORTOISE_ORM["connections"])))
                await conn.execute_query(
                    "UPDATE agent_pricing SET effective_to = NOW(6) "
                    "WHERE provider=%s AND model=%s AND unit_type=%s AND effective_to IS NULL AND id != %s",
                    [rec.provider, rec.model, rec.unit_type, rec.id],
                )
        except Exception as e:
            return _err(f"保存失败：{e}")

        # 模型配置表写后钩子：重载块定义 + 清全部缓存层（含 embed/seekdb 单例），改动立即全局生效
        ...
        # 单价表写后：失效 Billing 缓存，新价秒级生效（不等 60s TTL）
        if tname == "agent_pricing":
            await _invalidate_billing_cache_safe("save:agent_pricing")
        # 档位定义表写后：失效档位缓存 + 清 agent 实例，档位调整全员下条消息生效
        if tname == "agent_role_tier":
            await _clear_tier_cache_safe("save:agent_role_tier")

        await _audit(operator_id, f"save_record:{tname}", preview, f"{pk}={getattr(rec, pk)} granted={granted if tname == 'menus' else '-'}", True)
        return _ok({"table": tname, "mode": preview["mode"], "recordId": getattr(rec, pk), "changes": preview_changes})
    ...
    async def admin_delete_record(
        ...
        tname = _normalize_table(table)
        if tname in _READONLY_TABLES:
            return _err(f"拒绝：{tname} 是只读表（账务/运行流水，不可删除）。查询统计请用 admin_sql")
        if tname == "users":
        ...
        rec = await model.get_or_none(**{_pk_attr(model): record_id})
        if rec is None:
            return _err(f"{tname} 记录不存在：record_id={record_id}")
        ...
        if tname == "agent_pricing":
            preview["target"] = {"provider": rec.provider, "model": rec.model, "unit_type": rec.unit_type, "price_yuan": str(rec.price_yuan)}
            if rec.effective_to is None:
                preview["note"] = "该行是当前生效价，删除后该 (provider, model, unit_type) 条目将无价（后续用量费用记 0）。调价请用 admin_save_record 新建记录，不必删除"
        if tname == "agent_user_credit_quota":
            from app.settings import APP_SETTINGS

            preview["target"] = {"user_id": rec.user_id, "quota": rec.quota}
            preview["note"] = f"删除后该用户回落默认配额 {APP_SETTINGS.DEFAULT_CREDIT_QUOTA}（无记录=默认值），已用积分不受影响"
        if not user_confirmed:
            return _need_confirm(preview)
        ...
        # 单价行被删：同样失效 Billing 缓存
        if tname == "agent_pricing":
            await _invalidate_billing_cache_safe("delete:agent_pricing")
        # 档位行被删：同样失效档位缓存（引用该 code 的实体运行期 fail-open 当 'all'）
        if tname == "agent_role_tier":
            await _clear_tier_cache_safe("delete:agent_role_tier")
