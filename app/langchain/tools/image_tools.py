...
由 get_image_tools() 按 IMAGE 能力（load_provider("IMAGE")）动态选择暴露：
  - gpt：QuickRouter GPT-image（同步），支持文生图和图像编辑
  - apipod：APIPod GPT-image（异步任务式），支持文生图和参考图编辑/多图融合
  - 其它（qwen 等）：阿里云千问 Qwen-Image，支持文生图和图像编辑/多图融合

所有 provider 的具体模型版本一律由当前 IMAGE 预设块的 model 字段决定
...
async def generate_qwen_image(
    prompt: Annotated[
        str,
        "正向提示词。文生图建议 80~200 字，详细描述主体/风格/构图/光照；图像编辑时是编辑指令。prompt 长度有上限（Qwen-Image 系列约 1300 token），超长会被平台拒绝。",
    ],
    images: Annotated[
...
async def generate_apipod_image(
    ...
    """
    用 APIPod 中转的 GPT-image 生成或编辑图像（异步任务式，内部轮询直到出图）。
...
def get_image_tools() -> list[Any]:
    ...
    if not has_capability("IMAGE"):
        return []
    # with_lenient_args：兼容模型把 prompt_extend / watermark 等布尔参数传成字符串（"false"/"否"）
    from app.langchain.tools._tool_args import with_lenient_args

    provider = load_provider("IMAGE").provider
    if provider == "gpt":
        return [with_lenient_args(t) for t in [generate_gpt_image]]
    if provider == "apipod":
        return [with_lenient_args(t) for t in [generate_apipod_image]]
    return [with_lenient_args(t) for t in [generate_qwen_image]]
