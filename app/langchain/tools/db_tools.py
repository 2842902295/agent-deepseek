...
_CHAPTER_LIMIT_DEFAULT = 200  # 单次返回章节数默认值（推荐值，不硬拦；单章正文有 2000 字符截断 +
# 分页参数兜底，agent 需要更多章节时显式传大 limit 即可拿到；整体体积另有 MCP 桥 1400k 截断保底）

# standard_query 返回体积保护：
# 工具结果会作为 ToolMessage 驻留在之后每一轮的 LLM 上下文里，
# 裸 JSON 的 null/空串字段是 payload 滚雪球的主要来源（剥离冗余字段控体积）。
# 封顶值放宽到 1200k：旧值 30k 导致查询频繁被截断、agent 被迫多轮分批重查拼数据，
# 多轮往返的上下文总开销反而更大；超限截断时必须带命中总行数，agent 一次即可分批续取。
_QUERY_RESULT_CAP = 1200000  # 结果 rows 序列化后的字符数上限（超出按行截断）
...
def _like_escape(s: str) -> str:
    """转义 LIKE 通配符，防止章节号中的 % / _ 被当通配符。"""
    return s.replace("\\", "\\\\").replace("%", "\\%").replace("_", "\\_")


def _title_no_variants(p: str) -> list:
    """章节号层级展开的前缀变体。

    '附录A' 的子章节 title_no 实际存 'A.1'/'A.1.3'（不带「附录」前缀），
    故同时给出去掉「附录」的裁剪变体；其余章节号原样返回。
    """
    vs = [p]
    if p.startswith("附录") and len(p) > 2:
        vs.append(p[2:])
    return vs
...
def _resolve_images_dir() -> Path:
    ...
    if ctx and ctx.workspace_dir:
        ...
        # 兜底：按 uid 推导用户工作区（回合上下文缺失时），避免图片误落到项目根工作区
        uid = CTX_USER_ID.get()
        workspace = default_user_workspace(uid) if uid else _DEFAULT_WORKSPACE
...
_DEFAULT_QUERY_LIMIT = 2000  # 未显式写 LIMIT 时自动追加的行数上限（兜底防全表，不拦显式大 LIMIT）
...
def make_db_tools(pool_id: Optional[int] = None):
    ...
    async def standard_query(
        ...
        - 不写 LIMIT 会自动追加 LIMIT 2000（防全表兜底；需要更多行时显式写更大的 LIMIT 即可）
        """
    ...
    async def get_standard_chapters(
        ...
        limit: Annotated[int, "每次返回的章节数，默认 200；需要一次拿更多章节时直接传更大的值（配合 offset 分页亦可）"] = _CHAPTER_LIMIT_DEFAULT,
        ...
        **阅读策略：**
        ...
        - 传父章节（如 "4"/"附录A"/章节名）会自动取回该章本身及其全部子章节；不少标准的父章节自身正文为空、内容都在子章节里，**父章节正文为空不代表该标准没有内容**
...
async def _exec_query(sql: str, allowed_nos: Optional[list] = None) -> str:
    ...
    try:
        ...
        # 体积封顶：超限时保留前 N 行，并把命中总行数一并返回，
        # agent 据此用 LIMIT/OFFSET 一次续取剩余，不必盲目重查拼数据
        truncated = False
        total_rows = len(rows)
        if rows:
        ...
        if truncated:
            ...
            resp["note"] = (
                f"查询共命中 {total_rows} 行，体积超限，本次返回前 {len(rows)} 行。"
                "需要其余数据时用 LIMIT/OFFSET 从断点续取（字段多时可只 SELECT 需要的列），不要整批重查。"
            )
...
async def _exec_standard_chapters(
    ...
    try:
        ...
        # 章节数默认 30、不硬拦：agent 显式传大值时按需返回（单章正文另有 2000 字符截断控体积）
        limit = max(1, int(limit or _CHAPTER_LIMIT_DEFAULT))
        ...
        if title_no_prefix:
            # 容错："4." / "3.1." 这类尾随点写法剥掉后照常命中
            prefixes = [p.strip().rstrip(".") for p in title_no_prefix.split(",")]
            prefixes = [p for p in prefixes if p]
            or_parts = []
            for p in prefixes:
                # 层级展开：父章节号必须连带取回全部子章节。很多标准的父章节
                # （如 '3'/'附录A'）自身正文为空，内容全在子章节（'3.1'/'3.1.2.1'）里，
                # 不展开会让模型只拿到一个空章节，误判该标准没有内容。
                # title_no LIKE '4.%' 带点边界，不会误伤 '14'/'40' 等章节。
                like_parts = ["a.title_no = %s"]
                like_params: list = [p]
                for v in _title_no_variants(p):
                    like_parts.append("a.title_no LIKE %s")
                    like_params.append(_like_escape(v) + ".%")
                if re.search(r"\D", p):
                    # 含非数字字符（章节名，如"范围"）：追加 contains 兜底，
                    # 让不查目录直接传章节名也能命中（如 "范围" → "1 范围"）。
                    # 纯数字前缀不加，避免 "4" 误匹配 "14 xxx" 等章节。
                    or_parts.append(f"({' OR '.join(like_parts)} OR a.title = %s OR a.title LIKE %s OR a.title LIKE %s)")
                    params += like_params + [p, f"{p} %", f"%{p}%"]
                else:
                    or_parts.append(f"({' OR '.join(like_parts)} OR a.title = %s OR a.title LIKE %s)")
                    params += like_params + [p, f"{p} %"]
            if or_parts:
                # 按章节名命中的兜底展开：章节名（title LIKE）只能命中父章节自身，
                # 若父章节正文为空，需按其 title_no 把子章节一并展开。
                # 此处以 prefix 条件命中的章节 title_no 为锚点补子章节 LIKE 前缀。
                # 注意：必须在 keyword 条件追加进 params 之前执行，保证参数对齐。
                anchor_cond = " OR ".join(or_parts)
                anchor_sql = f"""
                    SELECT DISTINCT a.title_no
                    FROM standard_jgh_pdf_chapter a
                    JOIN standard_jgh_pdf b ON b.main_task_id = a.main_task_id
                    WHERE b.standard_no = %s AND ({anchor_cond})
                      AND a.title_no IS NOT NULL AND a.title_no != ''
                    ORDER BY a.title_no
                    LIMIT 30
                """
                async with standard_pool.acquire() as conn:
                    async with conn.cursor(aiomysql.DictCursor) as cur:
                        # params 首元素即 standard_no，与 anchor_sql 占位符一一对应
                        await cur.execute(anchor_sql, params)
                        anchors = [r["title_no"] for r in await cur.fetchall()]
                desc_parts: list = []
                seen: set = set()
                for tn in anchors:
                    # 脏数据会把整段标题存进 title_no（如 '附录A（资料性附录）xxx'），
                    # 超长锚点跳过，避免生成无意义的超长 LIKE 模式
                    if len(tn) > 30:
                        continue
                    for v in _title_no_variants(tn):
                        pat = _like_escape(v) + ".%"
                        if pat in seen:
                            continue
                        seen.add(pat)
                        desc_parts.append("a.title_no LIKE %s")
                        params.append(pat)
                if desc_parts:
                    conditions.append(f"({anchor_cond} OR {' OR '.join(desc_parts)})")
                else:
                    conditions.append(f"({anchor_cond})")
                    ...
                    if title_no_prefix or keyword:
                        ...
                        if base_total > 0:
                            toc_sql = f"""
                                ...
                                LIMIT 2000
                            """
                            await cur.execute(toc_sql, [standard_no])
