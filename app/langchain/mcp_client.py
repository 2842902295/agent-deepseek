...
当前接入：
  - 阿里云 DashScope WebSearch（联网搜索，Streamable HTTP）
  - Playwright（浏览器操作，仅 BRAND_VARIANT=standard 内网版）
  - 用户自添加的 MCP 连接器（agent_connector 表，按用户生效集动态加载，见 get_user_connector_tools）
...
async def get_mcp_tools() -> list:
    ...
    # ── DashScope WebSearch（Streamable HTTP） ────────────────────────────────
...
__all__ = ["get_mcp_tools", "build_connector_connection", "get_user_connector_tools"]


# ── 用户 MCP 连接器（agent_connector 表驱动，按用户生效集加载） ─────────────


def build_connector_connection(transport: str, url: str, api_key: Optional[str]) -> dict:
    """构造 langchain-mcp-adapters 的 connection 配置。

    transport 仅 sse / streamable_http（create_session 两者都认）；
    api_key 以 Authorization: Bearer 头发送。日志/响应不得出现 api_key。
    """
...
_CONNECTOR_LOAD_TIMEOUT_S = 10.0


async def get_user_connector_tools(rows, key_map: Optional[dict] = None) -> list:
    """加载用户生效连接器（AgentConnector 行）的工具。

    key_map: {connector_key: 解析后的凭据}（本人个人凭据 > 共享凭据，调用方解析）；
    缺省回落行上共享凭据 row.api_key。
    每个连接器独立 MultiServerMCPClient（tool_name_prefix=True → 工具名带
    `{connector_key}_` 前缀，防跨连接器及与内置工具重名）；并发加载 + 单 server
    超时兜底，个别失败跳过不阻断（与 get_mcp_tools 的 failure-safe 口径一致）。
    构建期只做一次 list_tools 握手，运行期按次拨号（库行为，同内置 websearch）。
    """
    if not rows:
        return []
    try:
        ...
        logger.warning("langchain-mcp-adapters 未安装，跳过用户连接器工具加载")
        return []

    async def _load_one(row):
        api_key = key_map.get(row.connector_key) if key_map is not None else row.api_key
        conn = build_connector_connection(row.transport, row.url, api_key)
        client = MultiServerMCPClient({row.connector_key: conn}, tool_name_prefix=True)
        tools = await asyncio.wait_for(client.get_tools(), timeout=_CONNECTOR_LOAD_TIMEOUT_S)
        for t in tools:
            try:
                t.handle_tool_error = True
            except Exception:
                pass
            _wrap_tool_safe(t)
            _attach_billing_callback(t, provider=f"connector:{row.connector_key}")
        return tools

    results = await asyncio.gather(*[_load_one(r) for r in rows], return_exceptions=True)
    out: list = []
    for row, res in zip(rows, results):
        if isinstance(res, BaseException):
            logger.warning(
                f"[MCP] 连接器工具加载失败 key={row.connector_key} "
                f"host={urlparse(row.url).netloc} err={type(res).__name__}"
            )
            continue
        out.extend(res)
    if out:
        logger.info(f"已加载 {len(out)} 个用户连接器工具：{[t.name for t in out]}")
    return out
