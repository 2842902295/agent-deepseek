...
class StandardJghPdfChapter(Model):
    ...
    title_no = fields.CharField(max_length=10240, null=True, description="章节编号")
...
class StandardJghPdfTerm(Model):
    """标准 PDF 术语表"""

    id = fields.BigIntField(pk=True, generated=False, description="术语ID")
    file_uuid = fields.CharField(max_length=100, null=True, description="文件UUID")
    main_task_id = fields.BigIntField(null=True, index=True, description="主任务ID")
    title = fields.CharField(max_length=255, null=True, description="术语名称")
    title_ename = fields.CharField(max_length=500, null=True, description="术语英文名称")
    chapter_no = fields.CharField(max_length=100, null=True, description="章节号")
    word = fields.TextField(null=True, description="术语内容")
    page = fields.IntField(null=True, description="页码")
    start = fields.IntField(null=True, description="字符起始index")
    end = fields.IntField(null=True, description="字符结束index")
    deleted = fields.BooleanField(default=False, description="软删除标记")

    class Meta:
        table = "standard_jgh_pdf_term"
        table_description = "标准PDF术语表"
