...
def _message_to_dict(m: AgentMessage) -> dict[str, Any]:
    d = {
        ...
        "processSteps": m.process_json or [],
        "status": m.status,
        "error": m.error,
...
class SessionCreateReq(BaseModel):
    ...
    expert_key: Optional[str] = Field(None, alias="expertKey", description="会话召唤的专家 key（专家面板「和 TA 对话」）；缺省=通用会话")
...
class SessionUpdateReq(BaseModel):
    ...
    expert_key: Optional[str] = Field(None, alias="expertKey", description="改绑/移除会话专家：传 key=改绑；空串=移除（驻留式召唤由 @专家 在 chat 侧改绑）")
...
async def list_sessions(
    ...
    # 附带会话所挂板的板型（前端侧栏徽标/伴侣面板恢复用）；只改 list 端点，不动 _session_to_dict
    wk_set = {d["workflowKey"] for d in data if d.get("workflowKey")}
    if wk_set:
        wf_rows = await AgentWorkflow.filter(workflow_key__in=list(wk_set), is_deleted=0).only("workflow_key", "board_type")
        bt_map = {w.workflow_key: w.board_type for w in wf_rows}
        for d in data:
            wk = d.get("workflowKey")
            if wk:
                # 板被软删时 join 不中 → 前端徽标容错（不给 boardType）
                d["boardType"] = bt_map.get(wk)
    # 附带会话召唤专家的展示信息（侧栏徽标/顶栏 chip 用）；专家被删时容错不回显
    ek_set = {d["expertKey"] for d in data if d.get("expertKey")}
    if ek_set:
        from app.models.standard.agent import AgentExpert

        ex_rows = await AgentExpert.filter(expert_key__in=list(ek_set)).only("expert_key", "name", "icon")
        ex_map = {e.expert_key: e for e in ex_rows}
        for d in data:
            ek = d.get("expertKey")
            if ek and ek in ex_map:
                d["expertName"] = ex_map[ek].name
                d["expertIcon"] = ex_map[ek].icon
...
async def create_session(req: SessionCreateReq):
    ...
    if req.expert_key:
        # 「和 TA 对话」：校验专家对当前用户生效（上架或本人、未被本人禁用），否则按通用会话建
        from app.api.v1.ai.agent_expert import get_effective_expert

        if await get_effective_expert(uid, req.expert_key.strip()) is not None:
            expert_key = req.expert_key.strip()
    key = _gen_key("sess")
    s = await AgentSession.create(
        session_key=key,
        user_id=uid,
        title=req.title or "新任务",
        thread_id=f"qa-{key}",
        expert_key=expert_key,
    )
    if expert_key:
        # 带专家建会话：失效技能同步节流，专家技能面随首条消息即时重算
        from app.api.v1.ai.agent_expert import _invalidate_skill_sync
...
async def update_session(session_key: str, req: SessionUpdateReq):
    ...
    if "expert_key" in req.model_fields_set:
        # 显式传了才动：空串/None=移除专家绑定；有值=改绑（校验生效口径）
        new_key = (req.expert_key or "").strip() or None
        if new_key:
            from app.api.v1.ai.agent_expert import get_effective_expert

            if await get_effective_expert(_current_user_id(), new_key) is None:
                return Fail(code="4004", msg="专家不存在或不可用")
        if new_key != s.expert_key:
            s.expert_key = new_key
            # 专家绑定变化 → 失效技能同步节流（专家技能面随下一条消息重算）
            from app.api.v1.ai.agent_expert import _invalidate_skill_sync

            _invalidate_skill_sync(_current_user_id())
    await s.save()
    d = _session_to_dict(s)
    if s.expert_key:
        from app.models.standard.agent import AgentExpert

        e = await AgentExpert.get_or_none(expert_key=s.expert_key)
        if e is not None:
            d["expertName"] = e.name
            d["expertIcon"] = e.icon
    return Success(data=d)
...
async def append_message(session_key: str, req: MessageAppendReq):
    ...
    m = await AgentMessage.create(
        ...
        process_json=req.process_steps,
        status=req.status,
        error=req.error,
    )
    # 维护会话 message_count / updated_at
    s.message_count = s.message_count + 1
    if req.role == "user" and (not s.title or s.title in ("新对话", "新任务")) and (req.content or "").strip():
