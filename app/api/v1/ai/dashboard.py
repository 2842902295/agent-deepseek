...
async def user_detail(
    ...
    # 凝练技能（按区间）
    skill_sql = (
        "SELECT id, skill_key, name, source, is_enabled, create_time "
...
async def session_messages(session_key: str):
    ...
    _, msg_rows = await conn.execute_query(
        "SELECT id, role, content, thinking, tool_steps_json, process_json, status, error, create_time "
    ...
    for r in msg_rows:
        messages.append({
            ...
            "toolSteps": _parse_json_col(r["tool_steps_json"]) or [],
            "processSteps": _parse_json_col(r["process_json"]) or [],
            "status": r["status"],
