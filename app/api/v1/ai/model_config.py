...
超管在 qa-glass 页面全局切换四类模型（chat / image / video / embed），切换对所有用户立即生效：
- GET  /ai/model-config   各类别预设清单 + 当前选中（不含 api_key）
- PUT  /ai/model-config   切换某类别选中的预设块，写库 + 刷新内存映射 + 清缓存

注意：切换 embed 块只改变「新请求用哪个向量空间」，既有向量库 / 知识库不会自动重嵌
（库按构建时快照判定陈旧，需显式重建）；前端对 embed 切换有加重确认文案。
...
class ModelConfigUpdate(BaseModel):
    """切换入参：category（chat/image/video/embed）+ selectedKey（预设块名）"""

    category: str = Field(..., description="类别：chat / image / video / embed")
...
async def set_model_config(payload: ModelConfigUpdate):
    ...
    # 刷新内存映射并清全部缓存层（含 embed / seekdb 单例），使新模型对所有用户立即生效
