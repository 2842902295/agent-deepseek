...
from app.api.v1.ai.agent_connector import router as agent_connector_router
from app.api.v1.ai.agent_expert import router as agent_expert_router
from app.api.v1.ai.agent_skill import router as agent_skill_router
from app.api.v1.ai.chat_mode import router as chat_mode_router
...
from app.api.v1.ai.role_tier import router as role_tier_router
...
from app.api.v1.ai.standard_sync import router as standard_sync_router
...
from app.api.v1.ai.vector_library import router as vector_library_router
...
from app.api.v1.ai.agent_workflow import public_router as agent_workflow_public_router
...
ai_router.include_router(agent_connector_router)
ai_router.include_router(agent_expert_router)
...
# 注册对话模式路由（用户偏好全员可用；/config 模式配置仅 R_SUPER）
ai_router.include_router(chat_mode_router)

# 注册用户档位清单路由（登录可读；档位定义写由 admin agent 经 agent_role_tier 表完成）
ai_router.include_router(role_tier_router)
...
# 注册标准库同步管理路由（指纹增量同步，仅管理员）
ai_router.include_router(standard_sync_router)
...
# 注册向量库管理路由（统一向量库体系管理端，读写全走 vector_hub）
ai_router.include_router(vector_library_router)
...
# 应用制作托管（URL 内嵌 htmlAppToken 门控，token 与登录态双向隔离）
ai_public_router.include_router(html_app_router)
# 共享工作流公开入口：share-view（免登录，校验分享开启 + 已发布后签发短期 share 态托管 token）
ai_public_router.include_router(agent_workflow_public_router)
# LLM 回环代理：dsh 运行时的模型路由经此转发并注入 thinking 参数（loopback 守卫）
from app.api.v1.ai.llm_proxy import router as llm_proxy_router

ai_public_router.include_router(llm_proxy_router)
