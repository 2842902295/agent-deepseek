...
def _l2_normalize(embeddings: np.ndarray) -> np.ndarray:
    """
    L2 归一化（行向量）。归一化后内积等价于余弦相似度——
    与旧 FAISS IndexFlatIP + normalize_L2 链路数值等价（2026-08-26 去 faiss 化）。
    """
    norms = np.linalg.norm(embeddings, axis=1, keepdims=True)
    norms[norms == 0] = 1.0  # 零向量防除零（归一化后仍为零向量，内积恒 0）
...
async def match_indicators_with_llm(
    ...
    # Step 3: 归一化（归一化后内积 = 余弦相似度）
    logger.info("Step 3: 归一化向量...")
    target_norm = _l2_normalize(target_embeddings)

    # Step 4: 搜索最相似的候选对（归一化内积全量打分 + top-k）
    logger.info(f"Step 4: 为每个源指标搜索top-{top_k}个候选...")
    source_norm = _l2_normalize(source_embeddings)
    all_sims = source_norm @ target_norm.T  # (n_source, n_target)
    k = min(top_k, len(target_indicators))
    indices = np.argsort(-all_sims, axis=1)[:, :k]
    similarities = np.take_along_axis(all_sims, indices, axis=1)
