...
async def sediment_entry(req: KBSedimentReq):
    """单条粒度：让 qa_agent 沉淀指定那条 assistant 消息。

    素材自包含：待整理的回复正文（连同紧邻的用户提问作上下文）直接拼进触发文本；
    session_key 传 None 走匿名沉淀——单条沉淀无需整段对话 transcript。
    """
    ...
    if not content:
        return Fail(msg="该消息没有可沉淀的内容")
    # 紧邻的用户提问作为最小上下文（回答常引用问题，单独看易失真）
    prev_user = (
        await AgentMessage.filter(session_id=msg.session_id, id__lt=msg.id, role="user")
        .order_by("-id")
        .first()
    )
    prev_text = ((prev_user.content or "").strip()[:2000]) if prev_user else ""
    context_part = f"用户当时的问题：{prev_text}\n\n" if prev_text else ""
    trigger = (
        "[系统触发] 把下面的这条 AI 回复整理进知识库。"
        "按 sediment skill 走，最后输出 <sediment-report> marker。\n\n"
        f"--- 待整理的 AI 回复 ---\n{context_part}{content[:20000]}"
    )
    outcome = await run_sediment(
        trigger_text=trigger, session_key=None, user_id=uid
    )
...
async def sediment_session_stream(req: KBSedimentSessionReq):
    """整段会话粒度：让 qa_agent 把这次对话整理进知识库（对话素材由 run_sediment 从 DB 注入）。"""
