...
from app.schemas.base import Fail, Success, SuccessExtra, CommonIds
from app.schemas.users import UserCreate, UserUpdate, UserSearch, validate_password_strength
from app.utils.pwd_crypto import DECRYPT_FAIL_MSG, decrypt_password_field

router = APIRouter()


async def _decrypt_user_password(user_in: UserCreate | UserUpdate, redis: AioRedis, required: bool) -> tuple[bool, str | None]:
    """解密 body 里的密码密文并做强度校验，写回明文。

    - required=True（建用户）：密码必填，空则返回失败
    - required=False（改用户）：password 为空表示不改密，直接放行
    成功返回 (True, None)，失败返回 (False, msg)。
    """
    if not user_in.password:
        return (False, "请输入密码") if required else (True, None)
    plain = await decrypt_password_field(redis, user_in.key_id, user_in.password)
    if plain is None:
        return False, DECRYPT_FAIL_MSG
    try:
        validate_password_strength(plain)
    except ValueError as e:
        return False, str(e)
    user_in.password = plain
    return True, None
...
async def _(user_in: UserCreate, redis: AioRedis):
    # password 为 RSA-OAEP 密文，解密+强度校验后赋回明文（建用户密码必填）
    ok, err = await _decrypt_user_password(user_in, redis, required=True)
    if not ok:
        return Fail(msg=err)
...
async def _(user_id: int, user_in: UserUpdate, redis: AioRedis):
    # password 为 RSA-OAEP 密文（为空表示不改密）；非空则解密+强度校验后赋回明文
    ok, err = await _decrypt_user_password(user_in, redis, required=False)
    if not ok:
        return Fail(msg=err)
