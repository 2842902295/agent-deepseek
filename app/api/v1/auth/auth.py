...
async def _ensure_default_datasets_for_new_user(user_id: int) -> None:
    """新用户默认添加并启用系统数据集（仅 standard 变体生效，函数内部门控）。

    纯增益动作，任何失败都不阻断注册主流程。
    """
    try:
        from app.mcp_bridge.datasets import ensure_dataset_prefs

        await ensure_dataset_prefs([user_id])
    except Exception as e:  # noqa: BLE001
        log.warning(f"新用户数据集默认启用失败（不影响注册，启动时会补）: {e}")
...
async def _(payload: UserRegister, redis: AioRedis):
    ...
    await _ensure_default_datasets_for_new_user(user_obj.id)
