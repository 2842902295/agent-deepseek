...
if __name__ == "__main__":
    ...
    # host/port 在下方经 os.getenv 读取，但 pydantic Settings 只把 dotenv 加载进配置对象、
    # 不会写进进程环境变量（app 还要等 uvicorn.run 才被 import），
    # 所以桌面单机版 .env 里的 DESKTOP_HOST/DESKTOP_PORT 必须在这里显式加载一次。
    # 与 Settings 同源（ENV_FILE 指定，默认 .env）；override=True 对齐 Settings
    # 「dotenv 优先级高于环境变量」的既定顺序（settings_customise_sources）。
    try:
